...
# Google Gemini CLI를 사용하여 프롬프트를 실행하는 유틸 스크립트
# 자연어 프롬프트에서 파일 경로를 추출하고 파일 내용을 읽어 LLM에 전달
# 사용법: ./call-gemini.sh "[자연어 프롬프트]"
...
OUTPUT_FILE="$OUTPUT_DIR/gemini-$TIMESTAMP.txt"
...
# Gemini CLI 호출 (YOLO 금지, Sandbox)
echo "Google Gemini 호출 중..." >&2
gemini --sandbox "$FULL_PROMPT" > "$OUTPUT_FILE" 2>&1
