...
세 종류의 문서는 서로 다른 관점을 서술합니다. 역할이 섞이면 작성 시점의 기록을 현재 정책으로 오독하거나, 구현 세부가 상위 문서를 흔들게 됩니다.

|  | Spec (`spec/`, 각 plugin의 스펙 문서) | Rules (CLAUDE.md, `.claude/rules/`) | Plan (`plan.md`, `plans/`) |
|---|---|---|---|
...
- 현재 동작·정책 판단은 spec과 코드로, 작업 방식 판단은 rules로 내립니다. Plan의 내용이 이들과 다르면 spec·rules·코드가 맞습니다.
- Plan 안의 상태 표기(예: "설계 단계", "구현 전 승인 필요")는 작성 당시의 상태입니다. 현재 상태는 코드·이슈·PR에서 확인합니다.
- **리팩토링 리트머스**: 동작·계약이 그대로인 리팩토링에 spec 본문을 고쳐야 한다면, 그 spec은 구현 세부에 결합된 것입니다. 구현 구조·경계에 대한 지속 원칙은 spec이 아니라 rules의 영역이고, 세부는 코드에 맡깁니다 (`related_paths` 등 경로 참조 갱신은 메타데이터 정비라 예외).

### 안티패턴

- ❌ Plan을 읽고 현재 동작·정책을 판단 → plan은 그 시점의 기록일 뿐. spec·rules·코드로 판단합니다.
- ❌ 구현이 plan과 달라졌다고 plan을 업데이트 → plan은 고치지 않습니다. 규격이 바뀌었으면 spec에, 지침이 바뀌었으면 rules에 반영합니다.
- ❌ 모듈 구조·경계 컨벤션을 spec에 서술 → 리팩토링마다 spec이 흔들립니다. 지속 원칙은 rules로 올립니다.
- ✅ Plan은 설계 배경·버린 선택지를 이해할 때만 참조하고, 구현 판단은 spec·rules·코드에서 내립니다.
