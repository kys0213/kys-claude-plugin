...
1. **의존성 역전 (DIP)**: 코어 로직은 외부 시스템(API, DB, CLI, 파일시스템)을 직접 호출하지 않는다. trait으로 정의된 Port를 통해서만 접근한다.
2. **인터페이스 분리 (ISP)**: Port는 역할별로 분리한다. 하나의 거대한 인터페이스가 아닌, 각 외부 시스템별로 독립적인 trait을 정의한다.
3. **테스트 우선 (TDD)**: 코어 로직 구현 전에 블랙박스 테스트를 먼저 작성한다. Mock/In-Memory 구현체로 시나리오를 주입하고, 최종 아웃풋을 기준으로 검증한다.
