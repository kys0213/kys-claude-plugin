...
# 결과 저장 디렉토리
RESULT_DIR=".team-claude/sessions/${SESSION_ID}/delegations/${CHECKPOINT_ID}/iterations/${ITERATION}"
mkdir -p "$RESULT_DIR"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if command -v osascript &> /dev/null; then
        osascript -e "display notification \"Checkpoint $CHECKPOINT_ID 통과!\" with title \"Team Claude: 검증 성공\" sound name \"Glass\""
    elif command -v notify-send &> /dev/null; then
        notify-send "Team Claude: 검증 성공" "Checkpoint $CHECKPOINT_ID 통과!"
    fi
    ...
    if [ "$ITERATION" -lt "$MAX_ITERATIONS" ]; then
        ...
        # 에스컬레이션 알림
        if command -v osascript &> /dev/null; then
            osascript -e "display notification \"Checkpoint $CHECKPOINT_ID 에스컬레이션 필요\" with title \"Team Claude: 개입 필요\" sound name \"Sosumi\""
        elif command -v notify-send &> /dev/null; then
            notify-send -u critical "Team Claude: 개입 필요" "Checkpoint $CHECKPOINT_ID 에스컬레이션"
        fi
    fi
fi
