...
# Same environment, transparent-proxy variant: origin stays a normal github.com
# URL and the proxy is applied through the environment instead, so the URL test
# above cannot see it. Auth comes from the proxy and GitHub access goes through
# MCP, so `gh` is not the auth path here either and its status says nothing.
if [[ -n "${CCR_AGENT_PROXY_ENABLED:-}" || -n "${CLAUDE_CODE_REMOTE_ENVIRONMENT_TYPE:-}" ]]; then
  exit 0
fi
