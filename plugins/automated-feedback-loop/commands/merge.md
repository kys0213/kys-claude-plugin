...
│  STEP 1: PR 목록 수집                                         │
│                                                               │
│  • .afl-result.json 파일들에서 PR URL 수집                    │
│  • 또는 gh pr list로 afl/* 브랜치 PR 조회                     │
│  • 의존성 순서대로 정렬                                       │
└───────────────────────────────────────────────────────────────┘
        │
        ▼
...
│                                                               │
│  각 PR에 대해:                                                │
│  1. git fetch origin afl/{task}                               │
│  2. git merge afl/{task} --no-commit                          │
│  3. conflict 확인                                             │
│     • 없으면 → 커밋 후 다음 PR                                │
...
━━━ 머지할 PR 목록 ━━━
  ...
  1. afl/coupon-model    → PR #123 (✅ 테스트 통과)
  2. afl/coupon-service  → PR #124 (✅ 테스트 통과)
  3. afl/coupon-api      → PR #125 (✅ 테스트 통과)
...
```
⚠️ Conflict 발생: afl/coupon-service
...
━━━ 결과 ━━━
  ...
  ✅ afl/coupon-model     자동 머지
  ✅ afl/coupon-service   3개 conflict 해결 (사용자 확인)
  ✅ afl/coupon-api       자동 머지
...
⚠️ 일부 PR이 아직 완료되지 않았습니다
  ...
  ✅ afl/coupon-model    PR #123 (merged)
  🔄 afl/coupon-service  PR #124 (open - 작업 중)
  ⏸️ afl/coupon-api      대기 중

머지를 진행하려면 모든 PR이 완료되어야 합니다.
/afl:loop-status 로 진행 상황을 확인하세요.
```

### 테스트 실패

```
❌ PR 테스트 실패: afl/coupon-service
