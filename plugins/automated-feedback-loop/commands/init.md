---
name: afl:init
description: Automated Feedback Loop 초기 설정 - 프로젝트 분석 및 환경 구성
argument-hint: ""
...
프로젝트를 분석하고 Automated Feedback Loop 환경을 구성합니다.
...
```bash
# Hook 스크립트 복사
cp -r {plugin_path}/hooks/scripts/* .afl/hooks/

# 실행 권한 부여
chmod +x .afl/hooks/*.sh
```
...
```
✅ Automated Feedback Loop 초기화 완료
...
AskUserQuestion({
  questions: [{
    ...
    options: [
      ...
      { label: "설정만 수정", description: "/afl:setup 실행" }
    ],
    multiSelect: false
  }]
})
