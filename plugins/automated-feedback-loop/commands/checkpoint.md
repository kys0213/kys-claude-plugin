...
# 특정 세션의 Checkpoint
/afl:checkpoint --list --session abc12345

# 특정 Checkpoint 검증
/afl:checkpoint validate coupon-api

# Checkpoint 추가 (대화형)
/afl:checkpoint add "새 API 엔드포인트 검증"

# Checkpoint 수정 (대화형)
/afl:checkpoint edit coupon-api

# Checkpoint 삭제
/afl:checkpoint delete coupon-api
```
...
상세: /afl:checkpoint show <id>
검증: /afl:checkpoint validate <id>
```
...
```bash
/afl:checkpoint add "rate limiting 검증"
```
...
✅ Checkpoint 추가됨: rate-limiting
  ...
  저장: .afl/sessions/abc12345/specs/checkpoints.yaml
```
...
/afl:checkpoint --list 로 전체 목록을 확인하세요.
```
