...
# 질문 저장
QUESTION_FILE=".afl/sessions/${SESSION_ID}/delegations/${CHECKPOINT_ID}/pending-question.json"
...
if command -v osascript &> /dev/null; then
    osascript -e "display notification \"Worker가 질문을 하고 있습니다\" with title \"AFL: 인간 개입 필요\" sound name \"Ping\""
elif command -v notify-send &> /dev/null; then
    notify-send -u critical "AFL: 인간 개입 필요" "Worker가 질문 중: $CHECKPOINT_ID"
fi
