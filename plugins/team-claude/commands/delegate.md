...
상세 로그: .team-claude/sessions/abc12345/delegations/coupon-service/
```
...
상세 분석: .team-claude/sessions/abc12345/delegations/coupon-service/
```
...
`.team-claude/sessions/{session-id}/checkpoints/{checkpoint-id}.json` 에서 checkpoint 정보를 읽습니다.
...
### 참고 문서
- `.team-claude/sessions/{session-id}/architecture.md`
- `.team-claude/sessions/{session-id}/contracts.md`
...
Write 도구로 `.team-claude/worktrees/{checkpoint-id}/CLAUDE.md` 에 저장합니다.
...
git push origin team-claude/{checkpoint-id}

# (선택) Worktree 정리
cd ../..
git worktree remove .team-claude/worktrees/{checkpoint-id}
```
...
✅ coupon-service 구현 완료
  ...
  시도 횟수: 2회
  브랜치: team-claude/coupon-service

  다음 단계:
  - /team-claude:delegate coupon-api (다음 checkpoint)
  - git merge team-claude/coupon-service (메인에 머지)
```
...
⚠️ coupon-service 에스컬레이션
  ...
  2. 수동 구현: 직접 .team-claude/worktrees/coupon-service 에서 작업
  3. Checkpoint 수정: 기준이 너무 엄격한지 확인

  상세 로그: .team-claude/worktrees/coupon-service/.team-claude-result/
```
...
│  1. Git Worktree 생성                                           │
│                                                                 │
│  git worktree add .team-claude/worktrees/coupon-service \               │
│    -b team-claude/coupon-service                                        │
│                                                                 │
│  결과:                                                          │
│  .team-claude/worktrees/coupon-service/                                 │
│    ├── (프로젝트 전체 복사)                                     │
│    ├── CLAUDE.md  ← Worker에게 전달할 지시서                    │
│    └── .team-claude-task.json  ← 메타데이터                             │
└─────────────────────────────────────────────────────────────────┘
        │
        ▼
┌─────────────────────────────────────────────────────────────────┐
...
│    "worktree_path": "/abs/path/.team-claude/worktrees/coupon-service",  │
