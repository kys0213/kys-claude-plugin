...
description: Team Claude 설정 위자드 - 대화형으로 설정 수정
...
# 특정 섹션만 설정
/team-claude:setup project
/team-claude:setup feedback
/team-claude:setup validation
/team-claude:setup notification
/team-claude:setup agents
/team-claude:setup server     # 서버 설정 및 설치
```
...
Team Claude 로컬 서버를 설치하고 설정합니다.

```
━━━ 서버 설정 ━━━

Team Claude 서버는 Worker Claude들을 관리하고 피드백 루프를 자동화합니다.
```
...
AskUserQuestion({
  questions: [{
    ...
    header: "Server",
    options: [
      { label: "시작", description: "team-claude-server를 백그라운드로 실행" },
...
```bash
# 환경 변수와 함께 서버 시작
Team Claude_PORT=7890 Team Claude_EXECUTOR=iterm nohup team-claude-server > ~/.team-claude/server.log 2>&1 &
