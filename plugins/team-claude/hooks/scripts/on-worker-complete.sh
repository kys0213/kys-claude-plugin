...
CHECKPOINTS_FILE=".team-claude/sessions/${SESSION_ID}/specs/checkpoints.yaml"
...
if command -v osascript &> /dev/null; then
    osascript -e "display notification \"Checkpoint $CHECKPOINT_ID 검증 시작\" with title \"Team Claude: Worker 완료\""
fi

# OS 알림 (Linux)
if command -v notify-send &> /dev/null; then
    notify-send "Team Claude: Worker 완료" "Checkpoint $CHECKPOINT_ID 검증 시작"
fi
