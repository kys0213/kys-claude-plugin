...
export class ITermExecutor implements WorkerExecutor {
  ...
  async execute(config: ExecutorConfig): Promise<ExecutorResult> {
    ...
    const markerFile = `/tmp/team-claude-task-${config.taskId}.done`;
    const outputFile = `/tmp/team-claude-task-${config.taskId}.output`;
    const exitCodeFile = `/tmp/team-claude-task-${config.taskId}.exitcode`;
  ...
  async cleanup(taskId: string): Promise<void> {
    ...
    // 임시 파일 정리
    await $`rm -f /tmp/team-claude-task-${taskId}.*`.quiet().nothrow();
  }
...
`.trim();
    ...
    // 탭 이름 설정
    const tabName = `Team Claude: ${config.checkpointName}`;
