...
2. **Minimal HITL**: 사용자 개입은 **설계 승인**과 **최종 PR 리뷰** 2곳만. 나머지는 풀자동
...
사용자 개입(Human-in-the-Loop)은 **Phase 1 설계**와 **Phase 4 최종 리뷰**에서만 발생합니다:
...
| HITL 지점 | Phase | Step | 목적 | 자동화 불가 이유 |
|-----------|-------|------|------|-----------------|
| **#1 요구사항 수집** | Phase 1 | Step 1.1 | 모호한 부분이 명확해질 때까지 반복 질문 | 사용자만 아는 맥락과 의도 |
| **#1 설계 승인** | Phase 1 | Step 1.5 | 아키텍처/Checkpoint 확인 | "무엇을 만들지"는 인간이 결정 |
| **#2 최종 PR 리뷰** | Phase 4 | Step 4.1 | RALPH 완료 후 코드 리뷰 결과 확인 | 머지 전 최종 품질 게이트 |

**나머지는 모두 자동**:
- Step 0 세션 재개: 자동 이어서 진행 (DONE이면 새로 시작)
- Phase 2 리뷰: Multi-LLM 리뷰 + 피드백 자동 반영
- Phase 3 구현: 전략 자동 선택 + RALPH 자동 재시도 (3회 실패 시에만 예외적 HITL)

> **자동화 품질 향상**: 에이전트가 자율적으로 올바른 판단을 하려면 맥락이 필요합니다.
> CLAUDE.md, `.claude/rules/`, 설정 파일에 프로젝트 컨벤션과 암묵지를 지속적으로 추가하세요.
> 축적된 맥락이 풍부할수록 HITL 없이도 정확한 결과를 얻을 수 있습니다.
...
사용자 요청
    ...
    ▼
┌─────────────────────────────────────────────────────────────┐
│  Phase 1: DESIGN (🔒 HITL #1)                               │
│  ├── 🔒 요구사항 수집 (반복 질문으로 맥락 확보)             │
...
사용자와 대화하며 모호한 부분이 **명확해질 때까지** 반복적으로 질문합니다.

**수집 절차**:
1. 사용자 요청 + CLAUDE.md/rules + 코드베이스를 먼저 분석
2. 분석 결과에서 모호하거나 부족한 부분을 식별
3. `AskUserQuestion`으로 구체적 질문 (선택지 제공)
4. 답변 반영 → 아직 모호한 부분이 있으면 다시 질문
5. 핵심 맥락이 충분히 확보되면 Step 1.2로 진행

**수집 항목**:
- 기능 요구사항 (핵심 기능, 사용자 시나리오)
- 비기능 요구사항 (성능, 확장성, 보안)
- 제약조건 (기술 스택, 팀 규모, 일정)
- **사이드이펙트 조사**: 변경으로 영향받는 기존 코드, 의존성, 동작
- 우선순위 (Must-have vs Nice-to-have)

**질문 원칙**:
1. 먼저 조사: 코드베이스/rules에서 알 수 있는 건 직접 파악
2. 모호한 것만 질문: 사용자만 알 수 있는 의도, 맥락, 우선순위
3. 선택지 제공: 열린 질문보다 구체적 옵션 제공
4. 반복: 명확해질 때까지 — 한 번에 끝내려 하지 않음
5. 충분하면 진행: 설계에 필요한 핵심 정보가 모이면 다음 단계로
...
> **자동 선택**: 전략은 에이전트가 자동으로 결정합니다. HITL 없음.
