...
> **예외 — 자율 모드(opt-in)**: 사용자가 자율 실행을 명시 활성화한 경우, 충돌 없는 머지의 자동 진행과 충돌 해결의 자동 위임이 가드레일 안에서 허용된다. 이 문서의 "보고 후 사용자 결정"은 **기본 모드 규칙**이고, 자율 모드에서는 `autonomous-driving.md`가 우선한다 (단 도메인 의미 충돌은 자율 모드라도 항상 에스컬레이션).
