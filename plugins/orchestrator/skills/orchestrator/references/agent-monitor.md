...
> **예외 — 자율 모드(opt-in)**: 사용자가 자율 실행을 명시 활성화한 경우에 한해, 가드레일(예산·종료조건·hard stop) 안에서 자동 재위임·SendMessage 주입이 허용된다. 이 문서의 "자동 개입 금지 / 재시도 1회"는 **기본(휴먼-인-더-루프) 모드 규칙**이고, 자율 모드에서는 `autonomous-driving.md`가 우선한다.
