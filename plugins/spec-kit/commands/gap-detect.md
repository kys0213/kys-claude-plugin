...
스펙 ↔ 코드 갭을 검출합니다. `/spec-review` 와 동일한 file-pair-observer (L1) + gap-aggregator (L2) + gap-auditor 백본을 사용하지만, 출력에서 **Code ↔ Spec Gaps** 섹션을 우선 표시하고 다른 관점은 보조로 노출합니다.

> **새 흐름**: file-pair-observer 가 spec 과 code 양방향 관찰을 한꺼번에 수행하므로 별도 `--reverse` 옵션은 더 이상 필요하지 않습니다 (`SPEC_ONLY` / `CODE_ONLY` / `PARTIAL` / `DIVERGENT` 분류로 자동 표현됨). L2 finding 은 gap-auditor 단일 게이트로 인용 + 의미 통합 감사를 거칩니다 (mechanical L2 인용 검증 단계는 gap-auditor 가 흡수).
...
`/spec-review` Step 6 과 동일. L2 출력 후 gap-auditor 에이전트를 spawn (sonnet, `run_in_background=false`) 하여 인용 정확성 (M-0) + 의미 적합성 (M-1~M-6) 을 단일 게이트로 감사하고, major == 0 이 될 때까지 (최대 3회) L2 에 fix request 를 보낸다.

기존 mechanical L2 인용 검증 단계는 **삭제** — gap-auditor 의 M-0 카테고리가 흡수.

루프 정책:
- major == 0 → 종료
- iter < 3 && 진전 있음 (직전 major finding ID 집합과 다름) → L2 에 fix request 보내고 재호출 (Step 5 재진입)
- 그 외 (iter == 3 도달 또는 진전 없음) → 잔여 major 를 drop log 로 사용자에게 노출

상세 알고리즘 / 입력 프롬프트 / fix request 형식 / 종료 조건 / 실패 모드는 `/spec-review` Step 6.1 ~ 6.6 참조.
