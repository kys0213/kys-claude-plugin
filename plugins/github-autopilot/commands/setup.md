...
#### PreToolUse: Stagnation Guard

worker 가 `autopilot task claim` 으로 task 를 잡기 직전, ledger 의 유사 task 그룹을 검사하여 같은 영역 / 같은 패턴 반복 실패가 임계에 도달하면 자동으로 redirect (또는 escalate) 합니다.
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "bash ${CLAUDE_PLUGIN_ROOT}/hooks/protect-stagnation.sh",
      "timeout": 10
    }
  ]
}
...
- 매 Bash tool 호출 직전 stdin 의 명령을 검사
- `autopilot task claim ...` 패턴이 아니면 즉시 통과 (overhead 거의 없음)
- claim 인 경우 `autopilot check stagnation --task <id>` 호출:
  - exit 0 → 정상 진행
  - exit 4 → 차단 (exit 2) + 새 방향 prompt 를 stderr 로 노출
  - exit 5 → 자동 escalate (`autopilot task escalate ...`) + 사람 개입 요청
- `autopilot` / `jq` 부재 등 환경 이슈는 best-effort 통과 (가드가 머지 자체를 막지 않음)

설정 후 `bash ${CLAUDE_PLUGIN_ROOT}/hooks/protect-stagnation.sh` 가 실행 가능한지 (chmod +x) 확인합니다.
...
| `protect-stagnation.sh` | PreToolUse (Bash) | task claim 직전 stagnation 감지 → redirect / escalate |
...
>
> 각 hook의 `command` 값에는 **`${CLAUDE_PLUGIN_ROOT}` 문자열을 리터럴 그대로** 기록해야
> 합니다. 가독성·검증을 위해 실제 절대경로(`/Users/.../plugins/cache/.../0.30.0/hooks/...`)로
> **절대 풀어 쓰지 마세요.** 변수를 expand해서 박으면 플러그인이 `0.30.0 → 0.30.1`처럼
> 업데이트될 때 버전 디렉토리가 사라져 hook 경로가 깨지고, 매 도구 호출마다
> `No such file or directory` hook error가 발생합니다. `${CLAUDE_PLUGIN_ROOT}` 해석은
> Claude Code 런타임에 맡깁니다.

#### Step 3.5: 등록 검증

Write 직후 `~/.claude/settings.json`을 다시 읽어 각 hook이 올바르게 등록되었는지 확인합니다:

```bash
cat ~/.claude/settings.json
```

- `guard-pr-base.sh`, `protect-stagnation.sh`, `check-cli-version.sh`를 호출하는 hook 항목이 존재하는가
- 각 `command`에 **리터럴 `${CLAUDE_PLUGIN_ROOT}`** 가 포함되어 있는가 (절대경로가 아님)

`${CLAUDE_PLUGIN_ROOT}` 대신 절대경로(`/Users/...` 또는 `/home/...`)가 박혀 있으면 해당
`command`를 `bash ${CLAUDE_PLUGIN_ROOT}/hooks/<script>.sh` 형태로 다시 교체한 뒤 저장합니다.
검증을 통과하지 못하면 설치 성공 메시지를 출력하지 않습니다.

> 참고: `autopilot preflight`가 hook 등록 누락을 감지하면 settings.json을 **수동으로 편집하지
> 말고** `/github-autopilot:setup`을 다시 실행하세요. 슬래시 커맨드 컨텍스트에서만
> `${CLAUDE_PLUGIN_ROOT}`가 안전하게 처리됩니다.
...
- PreToolUse (Bash) → protect-stagnation ✅
