...
> `check-cli-version.sh`는 plugin.json의 SessionStart hook으로 자동 등록됩니다 (설치 불필요).

#### Hook 설명

| Hook | 이벤트 | 등록 방식 | 동작 |
|------|--------|----------|------|
| `guard-pr-base.sh` | PreToolUse | setup 설치 | 설정된 base branch 외 PR 생성 차단 |
| `check-cli-version.sh` | SessionStart | plugin.json 자동 | 세션 시작 시 CLI 버전이 과거이면 업데이트 안내 |
...
- plugin.json → check-cli-version hook ✅ (자동 등록)
