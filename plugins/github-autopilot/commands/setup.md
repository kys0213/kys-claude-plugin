...
`.claude/settings.local.json` 파일의 `hooks.PreToolUse` 배열에 아래 항목을 추가합니다:
...
#### Step 4.5: CLI Version Check Hook 설치 (user scope)

`~/.claude/settings.json`에 SessionStart hook을 추가하여,
세션 시작 시 autopilot CLI 버전이 과거이면 업데이트를 안내합니다.

> user scope로 설치하므로 모든 프로젝트에서 동작합니다.
> `github-autopilot.local.md`가 없는 프로젝트에서는 자동으로 skip됩니다.

`~/.claude/settings.json` 파일의 `hooks.SessionStart` 배열에 아래 항목을 추가합니다:
...
| Hook | 이벤트 | scope | 동작 |
|------|--------|-------|------|
| `guard-pr-base.sh` | PreToolUse | project | 설정된 base branch 외 PR 생성 차단 |
| `check-cli-version.sh` | SessionStart | user | 세션 시작 시 CLI 버전이 과거이면 업데이트 안내 |

#### 주의사항

- **guard-pr-base**: 프로젝트 `.claude/settings.local.json`에 설치 (autopilot 프로젝트에서만 필요)
- **check-cli-version**: user `~/.claude/settings.json`에 설치 (모든 프로젝트에서 버전 체크)
...
### Hook
- .claude/settings.local.json → guard-pr-base hook ✅ (project scope)
- ~/.claude/settings.json → check-cli-version hook ✅ (user scope)
