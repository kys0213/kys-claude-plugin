...
동일 커밋 범위에 대해 이미 QA PR이 열려있는지 확인합니다:

```bash
# autopilot:qa 라벨이 붙은 open PR 조회
gh pr list --label "{label_prefix}qa" --state open --json number,title,headRefName --limit 20
```

현재 HEAD의 short hash (`git rev-parse --short HEAD`)가 기존 QA PR의 브랜치명(`feature/qa-{short-hash}`)에 포함되어 있으면 중복으로 판단하고 skip합니다.

```
기존 PR: feature/qa-a1b2c3  ←→  현재 HEAD: a1b2c3 → SKIP
기존 PR: feature/qa-d4e5f6  ←→  현재 HEAD: a1b2c3 → CONTINUE
```

중복인 경우 "이미 동일 범위의 QA PR이 존재합니다: #XX" 메시지 출력 후 종료.
...
### Step 7: 테스트 실행 (test-executor Agent)
...
- 기존 테스트를 수정하지 않음 (추가만)
- 동일한 커밋 범위에 대해 중복 실행 방지 (Step 3에서 기존 QA PR의 브랜치 hash로 확인)
- 프로젝트의 테스트 컨벤션을 따름
