...
default_intervals:
  ...
  qa_boost: "1h"
  work_ledger: "10m"
  release_stale: "30m"     # cron interval for stale-Wip recovery sweep
stale_wip:
  threshold: "1h"          # Wip claims older than this are reaped on each tick
monitor:
  poll_sec: 60
idle_shutdown:
  max_idle: 5
...
2. `CronCreate(cron: "{work_ledger_cron}", prompt: "/github-autopilot:work-ledger")` — gap-backlog / qa-backlog / ci-backlog ledger claim+dispatch (writer는 gap-watch / qa-boost / ci-watch, reader는 이 cron)
3. `CronCreate(cron: "{release_stale_cron}", prompt: "/github-autopilot:stale-task-review --before {stale_wip.threshold}")` — `autopilot task list-stale --before {stale_wip.threshold} --json` 으로 stale Wip 후보를 관찰한 뒤, `stale-task-reviewer` 에이전트가 task 별로 release / fail / escalate / leave alone 을 판단합니다. CLI는 결정적 관찰만 수행하고 (CLAUDE.md "책임 경계: CLI vs Skill/Agent"), 회수 결정은 에이전트가 내립니다. 빈 목록은 즉시 종료 — idempotent.
4. test_watch 배열이 비어있지 않으면 각 스위트별:
...
| Work Ledger | /github-autopilot:work-ledger | {interval} | {cron} |
| Stale Task Review | /github-autopilot:stale-task-review --before {stale_wip.threshold} | {interval} | {cron} |
