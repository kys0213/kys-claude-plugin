...
3. `CronCreate(cron: "{release_stale_cron}", prompt: "Bash: autopilot task release-stale --before {stale_wip.threshold}")` — orphaned Wip claim 회수 (worker crash / ctrl-C / worktree 파괴 복구). 매 tick마다 `updated_at`이 cutoff보다 오래된 Wip 태스크를 Ready로 되돌리고 attempts를 감소시킵니다.
...
9. `CronCreate(cron: "{release_stale_cron}", prompt: "Bash: autopilot task release-stale --before {stale_wip.threshold}")` — stale Wip 회수 (worker crash / ctrl-C 복구). cutoff(`stale_wip.threshold`)보다 오래된 `updated_at`을 가진 Wip 태스크를 Ready로 되돌립니다. exit 0 (idempotent — 회수 대상이 0건이어도 정상).
...
| Release Stale | autopilot task release-stale --before {stale_wip.threshold} | {interval} | {cron} |
...
| Release Stale | autopilot task release-stale --before 1h | 30m | */30 * * * * |
