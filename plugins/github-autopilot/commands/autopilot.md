...
| `TASK_READY epic=<E> task_id=<ID>` | — | `/github-autopilot:work-ledger --epic {E}` (해당 epic만 claim+dispatch — Ready task가 새로 생기거나 unblock될 때마다 즉시 처리, cron 폴링 불필요) |
| `STALE_WIP candidates=<JSON> epic=<E>` | — | `/github-autopilot:stale-task-review --candidates {JSON}` (이미 daemon이 list-stale로 필터링한 후보를 그대로 전달, skill은 list-stale 호출을 skip하고 stale-task-reviewer 에이전트로 바로 디스패치) |
| `EPIC_DONE epic=<E> total=<N>` | — | **로그만 기록** (TODO: feat-epic-flow epic에서 `/github-autopilot:epic-rollup {E}` 디스패치 예정 — 현재는 epic-rollup 커맨드 미존재) |
...
1. `CronCreate(cron: "{build_issues_cron}", prompt: "/github-autopilot:build-issues")` — GitHub issues를 직접 읽으므로 SQLite events에 신호가 없습니다. 폴링 유지.
2. test_watch 배열이 비어있지 않으면 각 스위트별:
   `CronCreate(cron: "{suite_interval_cron}", prompt: "/github-autopilot:test-watch {suite.name}")` — 시간 기반 스케줄 테스트.
...
8. `CronCreate(cron: "{work_ledger_cron}", prompt: "/github-autopilot:work-ledger")` — gap-backlog / qa-backlog / ci-backlog ledger reader (Step 2.5에서 스냅샷으로 보여주는 3개 backlog의 Ready task를 claim해 issue-implementer로 디스패치)
9. `CronCreate(cron: "{release_stale_cron}", prompt: "/github-autopilot:stale-task-review --before {stale_wip.threshold}")` — stale Wip 관찰 + 에이전트 리뷰 (worker crash / ctrl-C / worktree 파괴 복구). CLI (`autopilot task list-stale`) 는 cutoff 보다 오래된 Wip 후보를 read-only 로 반환하고, `stale-task-reviewer` 에이전트가 task 별로 release / fail / escalate / leave alone 을 결정합니다. exit 0 (idempotent — 후보가 0건이어도 정상). 결정은 컨텍스트 의존이므로 CLI 가 아닌 에이전트가 수행합니다 (CLAUDE.md "책임 경계").
...
| Work Ledger | /github-autopilot:work-ledger | 10m | */10 * * * * |
| Stale Task Review | /github-autopilot:stale-task-review --before 1h | 30m | */30 * * * * |
