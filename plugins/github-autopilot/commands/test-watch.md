---
description: "설정된 테스트 스위트를 실행하고 실패 시 이슈를 생성합니다"
...
설정된 테스트 스위트(e2e, 성능 테스트 등)를 실행하고, 실패 시 GitHub issue를 생성하여 autopilot 파이프라인에 연결합니다.

## 사용법

```bash
/github-autopilot:test-watch                    # 설정된 모든 스위트 1회 실행
...
- 주기적 실행은 `/autopilot`이 `run-loop.sh`를 통해 관리 (이 command는 1회 실행만 수행)
