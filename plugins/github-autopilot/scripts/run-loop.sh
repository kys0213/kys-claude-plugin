#!/usr/bin/env bash
# run-loop.sh — autopilot loop runner
#
# Usage:
#   run-loop.sh <command> <interval> [label_prefix] [idle_check] [log_dir]
#
# Arguments:
#   command      — slash command (e.g., "/github-autopilot:gap-watch")
...
#   log_dir      — directory for tick logs (optional)
#
# Features:
...
while true; do
  ...
  if [[ "$IDLE_CHECK" == "true" ]]; then
    idle_exit=0
    autopilot pipeline idle --label-prefix "$LABEL_PREFIX" >/dev/null 2>&1 || idle_exit=$?
