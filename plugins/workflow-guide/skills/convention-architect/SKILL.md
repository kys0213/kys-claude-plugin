...
## 8. Value Interview Questions

### 인터뷰 원칙

- **감지 우선**: 자동감지 가능한 항목은 먼저 채우고, 사용자에게는 확인만 받는다
- **스펙트럼 제시**: 트레이드오프는 양자택일이 아닌 스펙트럼으로 묻는다
- **3회 이내**: AskUserQuestion은 최대 3회. 각 질문에 여러 항목을 묶는다

### A. 프로젝트 맥락

| 항목 | 자동감지 소스 | fallback 질문 |
|------|-------------|--------------|
| 프로젝트 유형 | `package.json`의 `main`/`types` → 라이브러리, `bin` → CLI, deploy 스크립트 → 제품 | "이 프로젝트의 유형은? (제품/라이브러리/내부 도구/오픈소스)" |
| 팀 구성 | `.github/CODEOWNERS`, `git shortlog -sn` 커밋터 수 | "팀 규모는? (1인/소규모 2-5명/대규모 6명+)" |
| 주요 독자 | docs 구조, README 톤 (document-analyzer 결과) | "주요 사용자는? (개발자/PM·디자이너 포함/외부 고객)" |

### B. 엔지니어링 트레이드오프

충돌할 때의 우선순위를 스펙트럼으로 제시합니다.
...
| 항목 | 자동감지 소스 | fallback 질문 |
|------|-------------|--------------|
| 문서 독자 | document-analyzer 톤/설명 깊이 분석 | "문서 독자는? (개발자만/비기술직 포함/외부 사용자)" |
| 톤 | document-analyzer 종결어미/격식 분석 | "문서 톤은? (격식/캐주얼/기술적)" |
| 언어 | document-analyzer 언어 혼용 패턴 | "문서 언어는? (한국어/영어/혼용 시 규칙)" |

---

## 9. CLAUDE.md Section Templates
...
- 가독성 > 성능 (명백한 병목 제외)
- 명시적 중복 > 조기 추상화 (3회 반복 전까지)
- 안정성 > 속도 (핫픽스 제외)
- 통합 테스트 우선, 유닛은 복잡한 로직에만

## 문서화

- **톤**: 캐주얼하되 정확하게 (합니다체)
- **언어**: 한국어 기본, 기술 용어는 영어 유지
- **독자**: 비개발자가 읽을 수 있는 수준으로
- **용어**: 첫 등장 시 영어 병기, 이후 한국어만
```
...
### scaffold-rules에서의 LSP 활용

코드 구조 분석 시 LSP가 사용 가능하면 Glob/Grep보다 정확한 정보를 얻을 수 있습니다.

| LSP operation | scaffold-rules 활용 | 얻는 정보 |
|---|---|---|
| `documentSymbol` | 파일별 심볼 구조 | 레이어 패턴 (클래스/함수/인터페이스 비율) |
| `goToImplementation` | 인터페이스 구현체 탐색 | 추상화 수준 (DIP 준수 여부) |
| `findReferences` | 심볼 참조 추적 | 의존 방향, 모듈 경계 |
| `workspaceSymbol` | 워크스페이스 전체 심볼 | 전체 레이어 분류 |
...
LSP 결과로 프로젝트의 엔지니어링 성향을 추론할 수 있습니다:

| 관찰 | 추론 |
|------|------|
| interface/trait 파일이 구현체 대비 많음 | 추상화·명시성 중시 |
| 순환 참조 없음 (findReferences 기반) | 모듈 경계 엄격 |
| 테스트 파일에 mock/stub 구현이 풍부 | 테스트 격리 중시 |
| 타입 정의 파일이 별도 분리됨 | 계약 기반 설계 |
