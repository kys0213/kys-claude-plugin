---
name: outline
description: 요구사항 기반 상위 레벨 설계 - 3개 LLM으로 아키텍처를 분석하고 ASCII 다이어그램으로 도식화합니다
argument-hint: "[설계 요청 사항]"
allowed-tools: ["Task", "Glob", "AskUserQuestion"]
---

# 상위 레벨 설계 개요 커맨드

요구사항과 제약조건을 수집하고, Claude/Codex/Gemini 3개 LLM으로 아키텍처를 설계한 후 ASCII 다이어그램으로 도식화합니다.

## 핵심 워크플로우

```
Phase 0: 요구사항 수집
    │
    ▼
Phase 1: 아키텍처 설계 (3개 LLM 병렬)
    ├── Claude ─┐
    ├── Codex  ─┼─→ 취합
    └── Gemini ─┘
    │
    ▼
Phase 2: 통합 및 도식화
    └── ASCII 다이어그램 생성
```

## Phase 0: 요구사항 수집

사용자 요청을 분석하고, 모호하거나 부정확한 부분이 있으면 `AskUserQuestion`으로 명확화합니다.

### 수집할 항목

1. **기능 요구사항**
   - 핵심 기능은 무엇인가?
   - 사용자 시나리오는?

2. **비기능 요구사항**
   - 성능 요구사항 (동시 사용자, 응답 시간)
   - 확장성 요구사항
   - 보안 요구사항

3. **제약조건**
   - 기술 스택 (언어, 프레임워크, 클라우드)
   - 팀 규모/역량
   - 예산/일정
   - 기존 시스템 연동

4. **우선순위**
   - Must-have vs Nice-to-have
   - MVP 범위

### 모호함 감지 및 질문

사용자 요청을 분석하여 다음 유형의 모호함을 감지하고 `AskUserQuestion`으로 명확화합니다:

#### 1. 규모/성능 모호

**감지**: "대규모", "빠른", "많은 사용자" 같은 정량화되지 않은 표현

```
AskUserQuestion:
- header: "규모"
- question: "예상 동시 사용자 수는 어느 정도인가요?"
- options:
  - "100명 미만 (소규모)"
  - "100~1,000명 (중규모)"
  - "1,000~10,000명 (대규모)"
  - "10,000명 이상 (초대규모)"
```

#### 2. 기술 스택 미지정

**감지**: 기술 스택 언급 없이 기능만 요청

```
AskUserQuestion:
- header: "기술 스택"
- question: "선호하거나 이미 사용 중인 기술 스택이 있나요?"
- options:
  - "JavaScript/TypeScript (Node.js, React)"
  - "Python (Django, FastAPI)"
  - "Java/Kotlin (Spring)"
  - "제약 없음 (추천해주세요)"
```

#### 3. 기능 범위 불명확

**감지**: "~같은 시스템", "~비슷한 기능" 같은 모호한 참조

```
AskUserQuestion:
- header: "핵심 기능"
- question: "MVP에서 반드시 필요한 핵심 기능은 무엇인가요?"
- options:
  - "[구체적 기능 A]"
  - "[구체적 기능 B]"
  - "[구체적 기능 C]"
- multiSelect: true
```

#### 4. 상충되는 요구사항

**감지**: "빠르면서 저렴하게", "단순하지만 확장 가능하게" 같은 트레이드오프 상황

```
AskUserQuestion:
- header: "우선순위"
- question: "다음 중 가장 중요한 것은 무엇인가요?"
- options:
  - "개발 속도 (빠른 출시)"
  - "성능 (응답 시간, 처리량)"
  - "확장성 (미래 성장 대비)"
  - "비용 (저렴한 운영)"
```

#### 5. 기존 시스템 연동 불명확

**감지**: "기존 시스템과 연동", "현재 사용 중인" 언급 시

```
AskUserQuestion:
- header: "연동"
- question: "연동해야 할 기존 시스템이 있나요?"
- options:
  - "없음 (새로 시작)"
  - "기존 DB 연동 필요"
  - "외부 API 연동 필요"
  - "레거시 시스템 마이그레이션"
```

### 질문 원칙

1. **한 번에 1-2개 질문**: 너무 많은 질문으로 사용자 피로 방지
2. **구체적 옵션 제공**: 열린 질문보다 선택지 제공
3. **맥락 기반 질문**: 사용자가 언급한 내용 기반으로 후속 질문
4. **충분하면 진행**: 설계에 필요한 최소 정보가 모이면 Phase 1 진행

### 요청이 충분히 명확한 경우

다음 조건을 만족하면 질문 없이 Phase 1로 진행:
- 핵심 기능이 구체적으로 명시됨
- 규모/성능 요구사항이 정량화됨
- 기술 스택이 지정되거나 "추천해주세요"라고 명시됨

### 요구사항 정리 형식

```markdown
# 요구사항 정리

## 기능 요구사항
- [FR-1] 사용자 인증 (이메일/소셜 로그인)
- [FR-2] ...

## 비기능 요구사항
- [NFR-1] 동시 사용자 1000명 지원
- [NFR-2] ...

## 제약조건
- 기술 스택: TypeScript, React, Node.js
- 클라우드: AWS
- 팀 규모: 3명

## 우선순위
- Must: FR-1, FR-2
- Should: FR-3
- Could: FR-4
```

---

## Phase 1: 아키텍처 설계 (3개 LLM 병렬)

요구사항을 기반으로 3개 LLM에 설계 요청을 병렬 전달합니다.

### 설계 프롬프트 구성

Phase 0에서 정리한 요구사항을 포함하여 프롬프트 구성:

```
# 아키텍처 설계 요청

## 요구사항
[Phase 0에서 정리한 요구사항]

## 설계 요청
위 요구사항을 만족하는 아키텍처를 설계해주세요.

다음 항목을 포함해주세요:
1. 주요 컴포넌트와 책임
2. 컴포넌트 간 상호작용
3. 데이터 흐름
4. 기술 선택과 근거
5. 잠재적 리스크

구체적인 코드가 아닌 상위 레벨 설계를 제공해주세요.
```

### 3개 Agent 병렬 실행

```
Task(subagent_type="outline-claude", prompt=PROMPT, run_in_background=true)
Task(subagent_type="outline-codex", prompt=PROMPT, run_in_background=true)
Task(subagent_type="outline-gemini", prompt=PROMPT, run_in_background=true)
```

### 결과 대기 및 취합

3개 결과를 모두 받은 후 Phase 2로 진행합니다.

---

## Phase 2: 통합 및 도식화

3개 LLM의 설계 결과를 통합하고 ASCII 다이어그램으로 시각화합니다.

### 통합 분석

```markdown
# 설계 통합 분석

## 합의 사항 (3개 LLM 공통)
- [공통 컴포넌트]
- [공통 기술 선택]

## 의견 차이
| 항목 | Claude | Codex | Gemini |
|------|--------|-------|--------|
| DB   | PostgreSQL | MongoDB | PostgreSQL |
| ...  | ...    | ...   | ...    |

## 최종 권장사항
[3개 의견을 종합한 추천]
```

### ASCII 다이어그램 생성

컴포넌트 다이어그램과 데이터 흐름도를 ASCII로 표현합니다.

#### 컴포넌트 다이어그램 예시

```
┌─────────────────────────────────────────────────────────────┐
│                        Client Layer                         │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │  Web App    │  │ Mobile App  │  │   Admin     │         │
│  │  (React)    │  │  (RN)       │  │  Dashboard  │         │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘         │
└─────────┼────────────────┼────────────────┼─────────────────┘
          │                │                │
          ▼                ▼                ▼
┌─────────────────────────────────────────────────────────────┐
│                       API Gateway                           │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  Load Balancer / Rate Limiter / Auth                │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────┬───────────────────────────────────┘
                          │
          ┌───────────────┼───────────────┐
          ▼               ▼               ▼
┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│  User        │  │  Product     │  │  Order       │
│  Service     │  │  Service     │  │  Service     │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘
       │                 │                 │
       ▼                 ▼                 ▼
┌─────────────────────────────────────────────────────────────┐
│                      Data Layer                             │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐    │
│  │ User DB  │  │Product DB│  │ Order DB │  │  Cache   │    │
│  │(Postgres)│  │(Postgres)│  │(Postgres)│  │ (Redis)  │    │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘    │
└─────────────────────────────────────────────────────────────┘
```

#### 데이터 흐름도 예시

```
[User] ──▶ [Web App] ──▶ [API Gateway] ──▶ [Auth Service]
                              │                   │
                              │              ┌────┴────┐
                              │              ▼         ▼
                              │         [JWT Token] [Session]
                              │              │
                              ▼              ▼
                        [User Service] ◀────┘
                              │
                              ▼
                        [User DB] ──▶ [Response]
```

### 최종 출력 구조

```markdown
# 상위 레벨 설계 결과

## 1. 요구사항 요약
[Phase 0 결과]

## 2. 아키텍처 개요
[3개 LLM 통합 분석]

## 3. 컴포넌트 다이어그램
[ASCII 다이어그램]

## 4. 데이터 흐름도
[ASCII 다이어그램]

## 5. 기술 스택
| 레이어 | 기술 | 선택 근거 |
|--------|------|-----------|
| Frontend | React | ... |
| Backend | Node.js | ... |
| Database | PostgreSQL | ... |

## 6. 리스크 및 고려사항
- [리스크 1]
- [리스크 2]

## 7. 다음 단계
1. [구체적인 다음 단계]
2. [결정이 필요한 사항]
```

---

## 사용 예시

```bash
# 새 시스템 설계
/outline "실시간 채팅 시스템을 설계해줘"

# 제약조건 포함
/outline "React + Node.js로 e-commerce 플랫폼 설계. 동시 사용자 10만명"

# 기존 시스템 확장
/outline "현재 모놀리스를 마이크로서비스로 전환하는 설계"
```

## 주의사항

- **요구사항 우선**: 코드가 아닌 요구사항부터 시작
- **도식화 필수**: 모든 설계에 ASCII 다이어그램 포함
- **다양한 관점**: 3개 LLM 의견 차이도 명시
- **API 필요**: Codex, Gemini CLI 설치 필요
