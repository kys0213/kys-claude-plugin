...
fn g2_cache_produces_analysis_snapshot() {
    ...
    assert!(cache_dir.exists(), "cache directory should exist: {}", cache_dir.display());

    // analysis-snapshot.json should exist and be valid JSON
    let snapshot_path = cache_dir.join("analysis-snapshot.json");
    assert!(snapshot_path.exists(), "analysis-snapshot.json should exist");

    let snapshot_content = std::fs::read_to_string(&snapshot_path).unwrap();
    let snapshot: serde_json::Value =
        serde_json::from_str(&snapshot_content).expect("analysis-snapshot.json should be valid JSON");

    // Verify expected top-level keys
    let obj = snapshot.as_object().expect("snapshot should be a JSON object");
    ...
    assert!(obj.contains_key("toolTransitions"), "should have toolTransitions");
...
fn g2_cache_produces_session_summaries() {
    ...
    assert_eq!(summaries.len(), 1, "should have 1 session summary (multi_tool.jsonl)");
    ...
    let summary: serde_json::Value = serde_json::from_str(&summary_content)
        .expect("session summary should be valid JSON");

    let obj = summary.as_object().expect("summary should be a JSON object");
    ...
    assert!(obj.contains_key("toolUseCount"), "summary should have toolUseCount");
...
fn g2_cache_produces_index_json() {
    ...
    assert_eq!(obj["totalSessions"].as_i64(), Some(2), "should have 2 sessions");
