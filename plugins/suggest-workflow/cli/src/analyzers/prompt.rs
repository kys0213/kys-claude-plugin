...
pub fn analyze_prompts(
    ...
    let mut prompt_map: HashMap<String, (usize, f64, i64)> = HashMap::new();
    let mut earliest = i64::MAX;
    let mut latest = i64::MIN;

    for entry in entries {
        let normalized = entry.display.trim().to_lowercase();
        ...
        let data = prompt_map.entry(normalized).or_insert((0, 0.0, entry.timestamp));
        data.0 += 1;
        data.1 += decay_weight;
    ...
    let mut top_prompts: Vec<PromptFrequency> = prompt_map
        .into_iter()
        .map(|(prompt, (count, weighted_count, last_used))| {
