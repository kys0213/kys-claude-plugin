...
요구사항을 분해할 때는 plan 모드(`EnterPlanMode`)가 아니라 `brainstorm` 과 `grill` 을 우선 사용한다.
...
- **설계 승인 후 구현 → `orchestrator`**: 직접 Edit/Write로 구현하기 전에 `orchestrator` 로 위임한다. 독립 작업은 병렬 fan-out, 단계 의존은 파이프라인, 장기 작업은 agent-team으로 분해해 구현한다.

```
✅ 요청 → brainstorm/grill 로 요구사항·설계 분해 → 승인 → orchestrator 로 구현
❌ 요청 → plan 모드 → 직접 구현
```
...
- 구현은 `orchestrator`로 위임해 작업을 분해한다
