...
**채널 = 코드가 아니라 설정**입니다. 각 채널은 "실행할 명령(argv) + 무엇을 먹일지(템플릿)"의
선언이고, CLI는 이벤트를 결정적으로 렌더해 치환한 뒤 **셸 없이** spawn합니다. 새 채널 연결
(telegram-send, ntfy, mail, 자체 스크립트 등)은 설정 항목 하나 추가로 끝 — 코드 변경이 없습니다.

| 상황 | 채널 예시 |
|---|---|
| 자리에 없음 (폰으로 알아야 함) | Slack Incoming Webhook (curl) |
| 같은 머신에서 딴 작업 중 (다른 창/프로젝트) | OS 알림 배너 (notify-send / osascript) |
| 워처 세션/자동화가 반응해야 함 | JSONL append + Monitor(`tail -F`) (poll) |
| 그 외 무엇이든 | stdin/argv 받는 아무 CLI |
...
해석 순서: **`ATELIER_NOTIFY_CONFIG`(명시 경로) → `<project>/.claude/atelier-notify.json`
→ `~/.claude/atelier-notify.json`** (글로벌 — 한 번 설정하면 모든 프로젝트 세션에 적용.
webhook URL 은 시크릿이므로 프로젝트 파일로 둘 경우 gitignore 권장):
...
```jsonc
{
  "channels": [
    ...
    {
      "name": "desktop",                                  // Linux. macOS는 아래 참고
      "exec": ["notify-send", "{title}", "{body}"]
    },
    {
      "name": "sink",                                     // Monitor(tail -F)용 JSONL
      "exec": ["sh","-c","mkdir -p \"$HOME/.claude/atelier-notify\" && cat >> \"$HOME/.claude/atelier-notify/events.jsonl\""],
      "stdin": "{json}\n"
    },
    {
      "name": "perm-only-telegram",                       // 새 채널 = 설정 한 줄
      "exec": ["telegram-send", "--stdin"],
      "stdin": "{text}",
      "events": ["notification"]                          // 이 이벤트만 수신
    }
  ]
}
```

**스키마**: `exec`(필수, argv 배열) · `stdin`(선택, 템플릿) · `name`(선택, 리포트 라벨,
기본 `exec[0]`) · `timeoutSeconds`(선택, 기본 5 — 초과 시 CLI 가 kill) ·
`events`(선택, `ask_user_question` | `notification` 필터, 생략 시 전부).

**템플릿 변수** (argv 원소와 stdin 에서 사용 가능):

| 변수 | 내용 |
|---|---|
| `{title}` / `{body}` | 배너용 짧은 제목/본문 (첫 질문 + 외 N개 + 프로젝트) |
| `{text}` | 사람이 읽는 전체 메시지 (Slack 등) |
| `{json}` | 구조화 이벤트 한 줄 (`{"event":...,"questions":[...]}`) |
| `{cwd}` / `{event}` | 프로젝트 경로 / 이벤트 종류 |
| `{var#json}` | 값을 JSON 문자열 리터럴로 이스케이프 (JSON body 삽입용) |

**안전**: 명령은 셸을 거치지 않고 argv 그대로 spawn 되므로, 질문 텍스트에 어떤 문자가
와도 셸 문법으로 해석되지 않습니다. macOS 배너도 같은 이유로 이스케이프 대신 인자 전달을 씁니다:
...
위의 `sink` 채널(JSONL append)이 그 짝입니다 — 워처 세션에서 이벤트 파일을 tail 하면
다른 세션이 입력을 기다리기 시작하는 순간 라인 단위로 반응할 수 있습니다:
