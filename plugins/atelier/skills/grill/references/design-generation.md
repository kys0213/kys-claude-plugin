...
description: 심문할 설계가 아예 없을 때 무에서 설계를 만들어 내는 발산→수렴 활동. grill 스킬이 "검토 대상이 없다"고 판정했을 때 로드하는 내부 참조 문서이며, 사용자가 직접 호출하지 않는다.
version: 1.0.0
user-invocable: false
---

# 설계 생성 (design generation)
...
이 문서는 **활동(activity)**이다. 심문 자세가 "검토할 설계가 아예 없다"고 판정했을 때만 발동한다. 생성이 끝나면 자세로 복귀해 만들어진 설계를 심문받는다.

<HARD-GATE>
설계를 제시하고 사용자가 승인하기 전까지는 어떤 구현 스킬도 호출하지 말고, 코드를 쓰지 말고, 프로젝트를 스캐폴딩하지 말고, 어떤 구현 행동도 하지 마라.
</HARD-GATE>

## 안티패턴: "이건 너무 단순해서 설계가 필요 없다"

설계 생성이 필요하다고 판정된 요청은 크기와 무관하게 이 프로세스를 거친다. **"단순한" 프로젝트야말로 미검증 가정이 가장 큰 낭비를 만드는 곳이다.** 설계는 짧아도 된다(정말 단순하면 몇 문장이면 충분하다). 하지만 반드시 제시하고 승인을 받아야 한다.

반대로 모호함이 없는 소규모 변경은 여기까지 오지 않는다 — 심문 자세의 **의도 재진술**(무엇을·어디서·어떻게, 한 문장 + 확인)로 충분하다. 설계 생성을 요구하는 것은 크기가 아니라 **검토할 안이 없다는 사실**이다.
...
3. **명확화 질문** — `SKILL.md` 의 심문 자세를 그대로 따른다 (절차를 여기서 재서술하지 않는다)
...
digraph design_generation {
    ...
    "Plan Mode / orchestrator" [shape=doublecircle];

    "프로젝트 컨텍스트 탐색" -> "시각 질문 예상?";
    "시각 질문 예상?" -> "시각 자료 제안\n(별도 메시지, 다른 내용 금지)" [label="yes"];
    "시각 질문 예상?" -> "명확화 질문 (심문 자세)" [label="no"];
    "시각 자료 제안\n(별도 메시지, 다른 내용 금지)" -> "명확화 질문 (심문 자세)";
    "명확화 질문 (심문 자세)" -> "2-3 접근법 제안";
    "2-3 접근법 제안" -> "설계 섹션별 제시";
    "설계 섹션별 제시" -> "사용자 설계 승인?";
    "사용자 설계 승인?" -> "설계 섹션별 제시" [label="no, 수정"];
    "사용자 설계 승인?" -> "설계 문서 작성" [label="yes, 장기 스펙 필요"];
    "사용자 설계 승인?" -> "심문 자세로 복귀\n(생성물 심문)" [label="yes, 장기 스펙 불필요"];
    "설계 문서 작성" -> "심문 자세로 복귀\n(생성물 심문)";
    "심문 자세로 복귀\n(생성물 심문)" -> "설계 섹션별 제시" [label="빈틈 발견"];
    "심문 자세로 복귀\n(생성물 심문)" -> "사용자 리뷰?";
    "사용자 리뷰?" -> "설계 섹션별 제시" [label="수정 요청"];
    "사용자 리뷰?" -> "Plan Mode / orchestrator" [label="승인"];
}
```
...
- 단일 스펙에 담기엔 큰 프로젝트면 사용자와 함께 서브프로젝트로 분해한다: 독립적인 조각은 무엇인가, 서로 어떻게 연결되나, 어떤 순서로 만들어야 하나. 그 다음 첫 서브프로젝트를 정상 설계 플로우로 진행한다. 각 서브프로젝트는 자기만의 스펙 → 계획 → 구현 사이클을 가진다
...
- 이 지향은 atelier 의 SOLID 원칙(CLAUDE.md 코딩 원칙)과 동일하다 — 설계 단계에서 SRP/ISP/DIP 를 미리 적용하는 것
...
생성을 끝낸 뒤 스스로 최종 점검을 선언하지 않는다. `SKILL.md` 의 심문 자세로 돌아가 생성된 설계·스펙을 대상으로 다시 질문을 받는다. 심문에서 빈틈(placeholder, 섹션 간 모순, 과대 스코프, 두 갈래로 읽히는 요구사항)이 드러나면 설계 제시 단계로 되돌아간다.

**사용자 리뷰 게이트:**

스펙 문서를 작성했다면 진행 전에 사용자에게 리뷰를 요청한다:

> "스펙을 `<경로>` 에 작성하고 커밋했습니다. 구현 계획을 잡기 전에 검토하시고 수정할 부분이 있으면 알려주세요."

사용자의 응답을 기다린다. 수정 요청이 오면 반영하고 심문을 다시 돈다. 사용자가 승인한 뒤에만 진행한다.
...
- **생성과 검증의 분리** — 만든 사람이 최종 승인자가 되지 않는다
- **유연하게** — 말이 안 되면 되돌아가 명확히 한다

## 시각 자료

설계 생성 중 목업·다이어그램·시각적 대안을 보여주는 도구다. **도구이지 모드가 아니다** — 사용자가 동의해도 모든 질문이 시각 자료를 거치는 게 아니라, 시각적 처리가 도움이 되는 질문에만 쓸 수 있게 되는 것이다.
...
> Derived from [obra/superpowers](https://github.com/obra/superpowers) — MIT License, © 2025 Jesse Vincent.
