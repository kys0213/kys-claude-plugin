...
- **"일단 진행"은 해소가 아니라 보류다.** 사용자가 어떤 항목을 명시적으로 보류하면 그 항목은 **보류된 미결**로 합의 요약에 그대로 남고, spec으로 적을 때는 미결 사항(TBD) 섹션으로 격리된다. 보류된 미결이 남은 설계는 자율 구현의 입력이 될 수 없다 — orchestrator 는 spec 에 미결이 하나라도 있으면 구현 dispatch 전에 hard stop 한다 (`orchestrator` `SKILL.md §정지 조건` 의 `spec 확정 게이트` 행). 보류를 받아들이기 전에 이 결과를 사용자에게 알린다.
- **종료 직전에 목록을 다시 센다.** 미결 0을 확인하고 합의 요약에 `미결: 없음`을 명시한 뒤에만 종료한다. 이 한 줄이 없는 요약은 종료가 아니다.
- **사용자가 없는 협의체 컨텍스트에서는 "다시 질문한다"가 치환된다.** 이 자세가 sub-agent 로 주입되면 되물을 상대가 없다 — 그때는 자율 어댑테이션 규칙(`orchestrator` `references/contracts.md §대화 스킬의 자율 어댑테이션`)을 따라, 탐색으로도 닫지 못한 항목을 **명시적 가정**(심문 검증이 그 가정의 위험도를 판정한다)이나 **에스컬레이션 항목**으로 남긴다. 거기서의 "미결 0"은 그런 항목이 없다는 뜻이 아니라, **남은 항목이 전부 가정 또는 에스컬레이션으로 명시 기록됐고 소리 없이 닫힌 것이 하나도 없다**는 뜻이다.
...
- **코드 변경**: Plan Mode 로 *어떻게* 바꿀지 구현 계획을 세우거나, 작업이 여러 갈래로 나뉘면 `orchestrator` 로 위임한다. 이때 합의된 결정 목록이 orchestrator 의 **설계 승인 마커** 입력이 된다 — orchestrator 는 마커 없이 구현 에이전트를 dispatch 하지 않는다 (계약: `orchestrator` `references/contracts.md §설계 승인 마커`). 보류된 미결이 남아 있으면 자율 구현으로 넘기지 않는다 — 넘겨도 orchestrator 가 spec 확정 게이트에서 hard stop 한다.
