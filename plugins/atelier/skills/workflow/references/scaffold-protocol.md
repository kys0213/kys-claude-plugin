...
Agent({
  ...
  prompt: |
    ...
    각 파일은:
    - 레이어·언어 한정 규칙에 paths: frontmatter 포함 (안전·금지 등 상시 유지 규칙은 예외 — rules-design.md §재주입 한계)
...
전체 변경 사항을 보고합니다: CLAUDE.md = 나침반(항상 로드, 프로젝트 가치관), `.claude/rules/` = 지도(레이어별 컨벤션 — 로딩 모드는 각 파일의 `paths:` 유무가 정함, `rules-design.md` §2).
