...
description: 자율주행이 spec(DESIGN/concerns/flows)을 입력으로 구현을 시작할 때, 팀 모드로 검토자(spec↔구현 적합성)와 QA 매니저(spec↔테스트 적합성)를 상주시켜 worktree 코드를 계속 리뷰·개선하는 패턴. autonomous-driving.md 리뷰어·QA 게이트의 spec 특수화. orchestrator 스킬 내부 참조 문서.
version: 0.1.0
...
> **적용 범위**: **무거운 경로** 전용이다. 이 게이트는 worktree 코드를 머지 전에 검증하는 절차라, tracked 변경이 없는 경량 경로에는 검증 대상이 없다 (`SKILL.md §경로 판정 게이트`).
