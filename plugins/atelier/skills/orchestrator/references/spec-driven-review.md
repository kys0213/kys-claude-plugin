...
- spec 입력이 없으면(자유 구현·버그 수정 등) 이 게이트 대신 `autonomous-driving.md`의 **일반 리뷰어 게이트**를 쓴다 — QA 매니저(테스트↔spec) 차원은 검증할 spec이 없으면 성립하지 않는다.
...
- **검토자**: 자동 머지의 핵심 안전장치이고 spec↔구현의 미묘한 갭을 봐야 하므로 보통 더 강한 역량(`sonnet` 이상)을 둘 가치가 있다.
- **QA 매니저**: 커버리지 매핑은 비교적 기계적이라 더 가벼운 tier로 시작할 수 있으나, "빈 assert·항상 통과" 같은 의미 판정이 필요하면 올린다.
- 표준 heuristic을 벗어난 선택은 근거와 함께 decision log에 남긴다.
...
7. **spec 없는데 QA 게이트 강제**: 검증할 spec이 없으면 테스트↔spec 판정이 성립 안 됨 → 일반 리뷰어 게이트로 폴백.
