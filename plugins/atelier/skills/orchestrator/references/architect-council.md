...
요구사항 분해·검증의 품질이 다운스트림 전체를 좌우하므로 협의체 역할은 **위임 가능한 최상위 tier**로 시작한다. tier 표는 `delegation-patterns.md §모델 선택`, 역할 기준 원칙은 `SKILL.md §모델 라우팅 전략`이 단일 출처다 — 여기서 중복 정의하지 않는다.
