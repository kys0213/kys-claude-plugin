...
| 실행 형태 | `isolation:"worktree"` subagent | **team member 전용** — 필수 등급, 폴백 금지 (`SKILL.md §team mode 강제 등급`) |
...
- **사용 조건**: 진입 시 확정한 team 가용 판정(`SKILL.md §진입 시 체크 4`). 자문은 **필수 등급**이므로 가용인데 단발 subagent로 대체하면 **위반**이다 — 등급 기준과 위반 처리는 `SKILL.md §team mode 강제 등급`이 단일 출처다.
