...
3. 분해 — 복잡·모호하면 아키텍트 협의체로, 그 밖(확정 spec · 단순 요청)은 메인이 직접. 도출 결과와 근거를 기록한다 (`references/contracts.md §task 도출 계약`).
4. 실행 계획 — 병렬과 순차, 위임 형태, 격리 여부, 세울 게이트 관점을 표에서 읽는다 (`SKILL.md §기본값 표`).
