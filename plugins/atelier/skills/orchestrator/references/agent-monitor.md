...
**종단별 행동 계약**
- 취합에 반영 → 중복 판정은 보고 성격과 무관하게 모든 보고가 거친다. 편집 목적 보고면 수령 검증을 함께 돈다(→ D35). 증거 계약 판정은 조사·리서치 보고만 → D37
...
flowchart TD
  ...
  D35_n1(["→ D37"])
  D35_q1 -->|No · 편집 아님(조사·리서치·게이트 보고)| D35_n1
  D35_q1 -->|Yes| D35_q2
  D35_q2 -->|Yes| D35_t1
...
- → D37 → 조사·리서치 보고의 수용만 증거 계약이 판정한다 — 편집 보고는 이 갈래로 오지 않고, 중복 판정(→ D36)은 성격과 무관하게 모든 보고가 거친다

**근거**: 보고 내용만으로 성공을 단정하면 변경 0건인 실패가 성공으로 취합된다.

**후속**: → D33 → D22

## D38. fan-out 복원력 판정 (규모 · 체크포인트 · 재시도 · 폴백)
<!-- needs: D14 D33 D34 -->

**입력 신호**
- fan-out 규모가 임계 이상인가 (임계는 → C04) · 완료 알림에 대응하는 체크포인트 파일이 있고 형식이 맞는가
- 실패가 gateway time-out · API 오류 등 일시적 인프라 오류인가 · fan-out 인프라 재시도 예산이 남았는가 (값은 → C04)
- 실패한 조각이 읽기 전용인가 편집 조각인가 (→ D20)

```mermaid
flowchart TD
  D38_q1{"fan-out 규모가 임계 이상인가 (임계는 → C04)"}
  D38_q2{"조각의 체크포인트가 존재하고 형식이 맞는가"}
  D38_q3{"인프라 재시도 예산이 남았는가"}
  D38_q4{"실패한 조각이 읽기 전용인가"}
  D38_t1["체크포인트 기준으로 취합에 반영"]
  D38_t2["메인이 직접 읽어 채운다"]
  D38_q1 -->|No · 임계 미만| D38_n1(["→ D33"])
  D38_q1 -->|Yes| D38_q2
  D38_q2 -->|Yes| D38_t1
  D38_q2 -->|No · 알림만 왔거나 비어 있음| D38_q3
  D38_q3 -->|Yes| D38_n1
  D38_q3 -->|No · 소진| D38_q4
  D38_q4 -->|Yes| D38_t2
  D38_q4 -->|No · 편집 조각| D38_n1
```
...
- → D33 → 임계 미만 fan-out 은 체크포인트·재시도·폴백을 적용하지 않고 실패를 바로 이 판정으로 넘기며, 임계 미만 실패는 수정 재위임 축(→ D33 의 수정 재위임 종단)으로 처리한다. 어느 예산에서 어떤 형태로 다시 위임할지는 재위임 판정이 정한다. 격리 재사용 제약도 그쪽 공통 계약을 따른다
...
**종단별 행동 계약**
- fan-out 인프라 재시도 → 조각 단위로만 소모하며 임계 이상 fan-out 에만 쓴다(→ D38). 예산을 넘기면 폴백으로 넘긴다(→ D38)
...
**진행 중 — 자율은 침묵이 기본**
4. 자율 모드에서 정상 루프는 보고하지 않는다(결정은 기록에 계속 남긴다, → C05) — HITL 은 → D45 의 결정 지점마다 보고한다
...
10. 핸드오프 — 무거운 경로는 파일 경로, 경량은 이 보고에 포함 (→ C06)
...
16. 판정 근거 표기 — 위 항목의 근거를 적을 때는 항목 id(D##·C##·P##) 로 지목한다. 시작·진행 중 보고에도 같게 적용한다

**한 줄 근거**: 6·7·11·12·15 — 실패한 건을 빼거나 수치를 agent 의 말로 세면 사용자가 보는 완료율과 실제 산출물이 어긋나고, 머지 결과를 후보 단위로 적지 않으면 보류된 브랜치가 조용히 사라진다. 16 — 근거를 id 로 적어야 사용자가 같은 판정을 문서에서 되짚을 수 있다.
