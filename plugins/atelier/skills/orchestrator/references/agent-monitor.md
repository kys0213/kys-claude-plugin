...
4. **teammate에 편집 격리 기대**: team은 공유 checkout이라 worktree 격리가 없다 → teammate가 직접 편집하면 충돌/덮어쓰기. 편집은 `isolation:"worktree"` subagent로 위임 (team은 session 종료 시 자동 정리되므로 별도 정리 불필요).
