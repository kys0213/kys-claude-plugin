...
동일 agent가 동일 내용의 보고·완료 알림을 재전송하면 **첫 수신만 취합에 반영**하고 이후 수신은 무시한다. 같은 내용을 두 번 취합하면 수치·상태가 이중 계산된다 (실사례: 동일 보고 3회 재전송 + idle 알림 중복이 task 리셋과 겹쳐 조사 사이클 1회가 통째로 낭비됨).
...
"완료 알림이 매우 오래 안 옴"을 감으로 판정하면 폴백이 늦어 대기 시간만 소모된다 (실사례: agent 3개가 보고 없이 idle → 폴백 판단 지연 → 결국 메인이 직접 분석). 한도를 dispatch 시점에 정해 두면 idle이 추정이 아니라 판정이 된다.
...
- idle 확정 후: **자율 모드**에서는 해당 agent를 폐기(결과를 기다리지 않기로 확정)하고 §fan-out 복원력의 폴백 규칙(read-only 조각은 메인 직접 분석, 편집 조각은 조건 바꿔 재위임)으로 즉시 회부한다. **HITL 모드**에서는 §보고 형식으로 사용자에게 보고하고 결정을 받는다.
- 폐기한 agent의 보고가 뒤늦게 도착하면 폴백 결과와 대조해 **새 정보가 있을 때만** 반영한다 — 이중 취합 금지는 위 §중복 보고 감지와 동일.
- 폐기·폴백 발생 사실은 최종 보고에 명시한다 — "실패한 건도 누락 없이 보고" 원칙.
...
| idle (기대 완료 시간 내 무보고) | 폐기 확정 후 폴백 또는 조건 바꿔 재위임 (§idle 판정 — HITL이면 사용자 보고) |
