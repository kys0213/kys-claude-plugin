...
- **spawn 결과의 `agentId`를 보관한다** — `name`을 못 쓰는 런타임에서는 이것이 그 agent와 다시 대화할 유일한 지목자다

> **완료 알림은 "최종 결과"만 실어 온다 — 그 이전의 말은 채널이 있어야 도착한다.** sub-agent가 그냥 출력한 텍스트는 메인에 보이지 않으므로, 진행 보고·질문·부분 결과를 받으려면 dispatch prompt에 `SendMessage({to: "main", ...})` 지시가 들어 있어야 한다 (`delegation-patterns.md §필수 포함 요소` 11번이 단일 출처). 이걸 빠뜨리면 agent가 막혀서 묻고 싶어도 물을 수단이 없어 **임의로 가정하고 진행**하거나, 메인 입장에서는 완료까지 침묵으로 보인다.
...
> **전제 1 — 도구 확보**: `SendMessage`는 deferred tool이라 `ToolSearch`로 스키마를 확보해야 호출된다 (`SKILL.md §진입 시 체크 0`). 확보하지 않은 세션에서는 아래 허용 케이스조차 실행되지 않으므로, "규칙을 지켜서 안 보낸 것"과 "보낼 수 없어서 못 보낸 것"이 구분되지 않는다.
>
> **전제 2 — 이 절의 규율 대상은 한 방향뿐이다.** 아래 허용/금지는 **메인 → 집행 중인 agent 방향의 명령 주입**에만 적용된다. 다음 둘은 이 규칙의 대상이 아니며, 오히려 빠뜨리면 안 되는 것이다:
> - **agent → 메인 방향의 보고** (`SendMessage({to: "main", ...})`) — agent가 메인에게 말할 유일한 채널이다. 막을 이유가 없다.
> - **dispatch prompt에 보고 채널 지시를 넣는 것** — 명령 주입이 아니라 위임 계약의 일부다 (`delegation-patterns.md §필수 포함 요소` 11번).
>
> 이 구분을 놓치고 "SendMessage는 원칙적으로 금지"로 읽으면 보고 채널까지 빼게 되고, 그 결과는 규칙 준수가 아니라 **agent가 답을 못 하는 상태**다.
...
- 완료 알림이 매우 오래 안 옴 → 감으로 기다리지 말고 기대 완료 시간 기준으로 판정한다 (§idle 판정)
- 결과에 에러 메시지 포함
...
동일 agent가 동일 내용의 보고·완료 알림을 재전송하면 **첫 수신만 취합에 반영**하고 이후 수신은 무시한다. 같은 내용을 두 번 취합하면 수치·상태가 이중 계산된다 — 동일 보고 재전송이 idle 알림 중복과 겹치면 조사 사이클 하나가 통째로 낭비된다.

- **"동일"의 판정**: 같은 agent(agentId/name) + 같은 task에 대한 실질적으로 같은 결론. 새 정보가 한 줄이라도 있으면 중복이 아니라 갱신이다 — 갱신은 최신본으로 대체한다.
- 중복 수신은 취합 대상이 아니라 **이상 신호**다 — decision log에 남기고, 같은 agent에서 반복되면 §재위임 판단 기준(prompt 결함)으로 회부한다.

### idle 판정 — 무보고 대기의 한도

"완료 알림이 매우 오래 안 옴"을 감으로 판정하면 폴백 판단이 늦어, 대기 시간만 소모한 끝에 결국 메인이 직접 분석하게 된다. 한도를 dispatch 시점에 정해 두면 idle이 추정이 아니라 판정이 된다.

- **기대 완료 시간을 dispatch 시점에 정한다**: 스코프 대비 합리적 한도(예: 파일 몇 개 조사 = 수 분, 모듈 구현 = 수십 분)를 decision log에 남긴다.
- 한도를 넘겨 완료 알림도 중간 보고도 없으면 **idle로 확정**한다 — 추가 대기는 판단이 아니라 지연이다.
- idle 확정 후: **자율 모드**에서는 해당 agent를 취소한다 — 중단 도구가 있는 런타임에서는 실제로 중지시키고, 없으면 폐기(결과를 기다리지 않기로 확정)로 갈음한다. 이어서 §fan-out 복원력의 폴백 규칙(read-only 조각은 메인 직접 분석, 편집 조각은 조건 바꿔 재위임)으로 즉시 회부한다. **HITL 모드**에서는 §보고 형식으로 사용자에게 보고하고 결정을 받는다.
- 취소한 agent가 편집 조각(isolation:"worktree")이었다면 그 worktree 브랜치를 **머지 후보에서 제외**하고 merge-coordinator에 폐기 대상으로 표시해 넘긴다 — 방치하면 "변경 있음 → 머지 후보" 규칙이 버려진 브랜치를 주워 간다. 편집 조각을 재위임할 때는 원 agent가 실제로 멈췄는지(또는 그 결과를 버릴 것임)를 확인해 같은 파일을 두 agent가 동시에 편집하지 않게 한다.
- 취소한 agent의 보고가 뒤늦게 도착하면 폴백 결과와 대조해 **새 정보가 있을 때만** 반영한다 — 이중 취합 금지는 위 §중복 보고 감지와 동일.
- 취소·폴백 발생 사실은 최종 보고에 명시한다 — "실패한 건도 누락 없이 보고" 원칙.
...
| idle (기대 완료 시간 내 무보고) | "원인 불명확" 행보다 우선한다 — idle은 기대 시간 기준으로 판정 가능한 신호다. 취소 후 폴백 또는 조건 바꿔 재위임하며, 재위임은 max_redispatch_per_task를 소모한다 (§idle 판정 — HITL이면 사용자 보고) |
...
- [ ] prompt에 보고 채널(`SendMessage({to: "main", ...})`)을 넣었는가? (§백그라운드 위임 기본형 — 빠지면 agent가 답할 수단이 없다)
- [ ] spawn 결과의 `agentId`(또는 `name`)를 보관했는가? (없으면 다시 대화할 수 없다)
...
- [ ] 정상 진행은 침묵하고 있는가?
- [ ] 정체/실패만 사용자에게 보고하는가?
- [ ] 자동으로 **메인 → 집행 agent 방향의** SendMessage를 보내려 하고 있지 않은가? (사용자 결정 우선 — 단 자문 반문은 허용. agent → 메인 보고는 이 규칙의 대상이 아니다)
- [ ] dispatch마다 기대 완료 시간을 정해 decision log에 남겼는가? (§idle 판정 — 없으면 idle을 판정할 기준이 없다)
- [ ] 동일 agent의 동일 내용 재전송 보고를 이중 취합하지 않는가? (§중복 보고 감지)
...
- [ ] idle로 취소한 agent와 그 폴백 결과를 최종 보고에 명시했는가? (§idle 판정)
