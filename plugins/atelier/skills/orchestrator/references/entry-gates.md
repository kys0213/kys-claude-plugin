...
- 경량 경로 → **적용하지 않는 항목은 다음뿐이다** — 토폴로지 가드(→ D21 · → D22) · 격리(→ D20 은 격리 없음 종단) · 충돌 사전 분석(→ P10 · → D13) · 머지 조정(→ D39 · → D40 · → D41 · → D42 · → D43 · → D44 · → P07 · → P08 · → C10 · → C11 · → C12) · 재분해(→ D12 는 편집 런만 — 조사 런의 분해는 → D15). 리뷰·QA 발동 여부와 축소 범위는 → D30 이 판정한다 — 이 블록이 게이트 실행 여부를 단정하는 것은 금지한다. 그 밖은 그대로 유지한다 — 조율 도구 확보(→ C01) · 왕복 조율 판정(→ D04) · Task 등록 판정(→ D54) · preflight(→ D06) · 복원력(→ D38) · 취합 보고(→ C13). **산출 경로 계약을 prompt 에 싣는다** (→ D56). 경량 경로에는 epic 브랜치가 없으므로 기록 위치의 `<epic>` 자리는 런 식별자로 대체한다 — 산출 경로·체크포인트 범위는 → D56, 기록 형식은 → C05 · → C06. 외부 시스템 쓰기만 하는 런도 되돌림 비용이 있으므로 preflight 와 에스컬레이션(→ D47)은 유지한다
...
- 종료 조건(`done_when`)을 판정 가능한 형태로 세울 수 있는가 — 명령(테스트·빌드·lint·git 상태) 또는 산출물 수용(→ D36 · → D37, 지정 경로의 산출물 존재)
- 기본값은 자율 주행이며, 판정은 진입 1회 확정이다

```mermaid
flowchart TD
  D45_q1{"단계별 확인을 명시했는가"}
  D45_q2{"done_when 이 판정 가능한가 — 명령 또는 산출물 수용"}
...
**종단별 행동 계약**
- HITL → 정체·실패·충돌은 보고하고 결정을 기다린다. HITL 분기를 가진 항목(그래프 노드 또는 종단 계약 — → D46 · → D42 · → D51 · → D31 · → D34)은 그 분기를 따른다 — 모드는 그 항목들의 판정 입력이다. 어느 쪽에도 분기가 없는 머지 실행(→ D40)과 재위임(→ D33)은 **실행 전에 사용자 결정을 받는다** — 분기 없는 항목을 자동으로 실행하는 것은 금지한다
...
2. 쿼리 대상은 deferred 조율 도구뿐이다. `Agent`·`Read` 처럼 처음부터 호출 가능한 비-deferred 도구는 확보 대상이 아니다 — 결과 목록에 없다고 부재로 판정하는 것은 금지한다. 그 스키마는 이미 있는 것으로 보고 판정한다 (→ D04 의 `Agent` 신호).
3. 세션당 1회만 한다. 확보된 스키마는 세션 내내 유효하다.
4. 결과 목록에 없는 **deferred** 도구는 이 런타임에 없는 것이다 — 이름을 추측해 호출하는 것은 금지한다. 없는 경로는 폴백으로 대체한다.
5. `SendMessage` 미확보의 결과는 → D04 의 첫 노드가 판정한다.
6. 이 체크는 공유 전제 판정의 한 사례다 — 조율 경로 전체가 이것 하나에 걸려 있어 다른 모든 체크보다 앞선다 (→ D06).
7. 경량 경로에서도 생략하지 않는다 (→ D03).
...
2. **경로 판정** → D03. 3 다음, 무거운 경로면 4·5·6 을 거치고 경량 경로면 바로 7 로 간다.
3. **모드 판정** → D45. 토폴로지 확인이 모드를 판정 입력으로 쓰므로 그 앞에 온다.
4. **epic 브랜치 확보** (무거운 경로만) — 현재 브랜치가 epic 이면 그것을 이 런의 epic 으로 쓴다. 아니면 (a) 하네스·PR 이 지정한 작업 브랜치가 있으면 그 브랜치를 이 런의 epic 으로 삼고(그 이름이 `epic/` 접두가 아니어도 그대로 `<epic>` 이 된다 — 이름 규약은 → C10), (b) 없으면 자율은 `epic/<name>`(이름 규약은 → C10)을 만들고 HITL 은 어느 epic 으로 진입할지 사용자 결정을 받는다. 확보하지 않은 채 다음 단계로 가는 것은 금지한다 — 토폴로지·머지·기록 경로가 모두 이 이름을 전제한다. 확보한 이름과 확보 방식(기존·지정·생성)을 진입 보고와 decision log 에 남긴다 (→ C13 · → C05).
5. **토폴로지 확인** (무거운 경로만) — 4 에서 확보한 epic 브랜치이고, 메인이 worktree 가 아니라 메인 working tree 에서 clean 한가 → D22. 진입 이후에도 런 중 가드로 계속 돈다.
6. **격리 판정** (무거운 경로만) — 저장소 안 편집은 격리 위임으로 간다 → D20.
7. **왕복 조율 가용 판정** → D04.
8. **공유 전제 preflight** → D06. dispatch 직전 마지막 확인이다.
9. **일감 등록 판정** → D54, 등록하면 → C16.
10. 각 단계의 **판정 결과와 사용한 신호**를 진입 보고 1줄과 decision log 에 남긴다 (→ C13 · → C05). 근거 없는 생략은 판정이 아니다 — 생략하려면 그 판정을 먼저 남긴다.
