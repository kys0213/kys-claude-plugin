...
요지(기본값 뒤집힘·3관점 이상·순차 시작 금지)는 SKILL.md 본문이 갖고, 출구 판정·관점 수 기록의 상세는 이 절이 단일 출처다.

위 트리의 "의심스러우면 순차"는 **충돌 비용** 때문에 있는 규칙이다. read-only 조사·감사·원인분석은 대개 충돌 축(파일 집합·외부 리소스)에 걸리는 것이 없고, 그렇다고 판정되면 기본값이 뒤집힌다 — **첫 행동이 관점별 병렬 fan-out이다.** 충돌 대상이 없는 조사를 순차로 시작할 근거로 "의심스러우면 순차"를 끌어오지 않는다 — "일단 훑어보자"로 메인이 순차 탐색을 시작하면 사용자가 중단하고 병렬로 재지시하는 반복 마찰이 생긴다 (`SKILL.md §안티패턴` 18).
...
발동 시점·판정 기준(깨졌을 때 fan-out 전체가 죽는 공유 의존인가)·방법(read-only 확인)과 실패 시 1차 대응의 요지는 `SKILL.md §진입 시 체크` 5번이 소유한다. 이 절은 대상 도출·방법 상세·경계의 단일 출처다.
...
전환 트리거(계획에 없던 tracked 파일 편집이 필요해진 순간)와 역방향(무거운 → 경량) 전환 금지는 `SKILL.md §경로 전환`이 본문에 둔다. 5단계 절차·기록 규칙은 이 절이 단일 출처다.
...
위 표는 이 세 질문의 결과를 캐시한 열거다. 표에 없는 작업이나 경계가 애매한 작업은 표를 늘리지 말고 이 질문으로 판정한다:

1. **파급 범위 — 산출물이 다른 작업들을 구속하는가?** 분해·아키텍처·인터페이스 결정은 틀리면 그 위의 하위 작업 전부가 같이 틀린다 ("메인의 판단이 스웜 결과의 상한을 결정한다"와 같은 축 — `model-routing.md §역할 기준 원칙`). 반면 task 하나의 구현 실패는 그 task 재위임으로 끝난다.
2. **개방성 — 탐색 공간이 열려 있는가?** 설계 심문·어려운 디버깅은 정답 후보가 열려 있어 판단력이 결과를 좌우한다. 승인된 계획이 있는 구현은 "무엇을 할지"가 확정되어 실행 충실도만 남는다 — 계획 우선 게이트(§계획 우선 게이트)를 통과한 구현이 중간 tier로 충분한 대표 사례다.
3. **회수 비용 — 이 단계가 마지막 방어선인가?** 구현 실수는 뒤의 리뷰어·QA 게이트가 잡는다 (`autonomous-driving.md §리뷰어·QA 게이트`). 분해 판단이나 자동 머지의 리뷰 게이트 자체는 뒤에 잡아줄 것이 없어 강한 역량을 둘 가치가 있다.

**판정 규칙**: 하나라도 강하게 걸리면 최상위, 셋 다 아니면 중간 이하. 단 넓게 읽고 추려 오는 작업(discovery)은 역량이 아니라 범위·병렬성이 leverage라 경량이다 (아래 discovery 불릿). 판정이 표와 어긋나면 판정이 이기고, 그 선택은 근거와 함께 decision log에 남긴다.
...
- 이 표는 고정값이 아니라 **시작 heuristic**이다 — 작업마다 "지금도 이 역량이 필요한가"를 재평가하고, 벗어난 선택은 근거와 함께 decision log에 남긴다. 재평가의 질문이 위 §tier 판정 3질문이다. 자율 루프에서의 배분 원칙은 `autonomous-driving.md §모델 분배` 참조.
