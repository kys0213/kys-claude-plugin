...
경량/무거운 경로의 판정 기준과 유지·생략 규칙은 `SKILL.md §경로 판정 게이트`가 단일 출처다. 이 절은 기준만으로 바로 갈리지 않는 케이스의 판정을 정한다.

| 케이스 | 판정 |
|---|---|
| 조사 리포트를 `.md` 파일로 남긴다 | 경로가 repo 밖·gitignore면 **경량**, tracked 경로면 **무거운** — 축은 파일을 쓰는지가 아니라 **머지 경로에 올라타는지**다 |
| 조사 후 같은 세션에서 구현으로 이어진다 | 진입은 **경량**, 첫 편집 dispatch 직전에 **전환**한다 (`SKILL.md §경로 전환`) |
