...
- **단, fable 배분 정책은 정책이지 heuristic이 아니다**: 아키텍트 협의체(brainstorm·grill)는 최상위 tier floor를 유지하고, 협의체 외 모든 역할은 최상위 tier를 쓰지 않는다(ceiling). 이 envelope은 재평가 대상이 아니며, envelope **안에서의** tier 선택만 위 원칙대로 판단한다. 정책 전문은 `delegation-patterns.md §fable 배분 정책`이 단일 출처다.
...
```
구현 완료 → 검토 + QA (+ DB 접촉 시 DBA) 병렬 검증    # 독립 차원 동시
  전부 pass → 머지 후보로 승급 (이후 merge_coordinate)
  하나라도 reject → 재위임 (findings를 prompt에 포함, max_redispatch_per_task 예산 소모)
...
- 검토·QA·DBA 모델도 위 *모델 분배* 원칙으로 메인이 작업 리스크에 맞춰 정한다 (자동 머지의 유일한 안전장치이므로 보통 더 강한 역량을 둘 가치가 있으나, 고정은 아님 — 단 fable 배분 정책의 ceiling 안에서).
...
16. **고정 모델 매핑 박기**: "구현은 항상 X, 리뷰는 항상 Y"로 못 박음 → 모델이 똑똑해져도 비효율 유지. 매 dispatch마다 작업 리스크에 맞춰 재평가. 단 **fable 배분 정책(아키텍트 floor / 그 외 ceiling)은 매핑이 아니라 정책 envelope**이라 재평가 대상이 아니다 (`delegation-patterns.md §fable 배분 정책`).
...
- [ ] 각 작업의 모델을 작업 리스크에 맞춰 배분하고, 비표준 선택은 기록하는가? (fable 배분 정책 envelope 준수 — 아키텍트 floor / 그 외 ceiling)
