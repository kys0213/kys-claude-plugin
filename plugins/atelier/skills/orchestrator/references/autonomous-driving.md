...
- spec 확정 (spec_resolved): spec 입력 런이면 진입 전 미결 스캔 결과 — 스캔한 spec 경로 + 미결 0 확인
...
| **QA 에이전트 (qa)** | 요구사항 + worktree 테스트 코드 | 각 요구사항·flow·엣지케이스에 대응 검증 테스트가 있나? 테스트가 의도를 실제로 검증하나(빈 assert·항상 통과 아님)? **레포의 기존 테스트 하네스·픽스처를 사용하나**(ad-hoc 검증 아님)? 없으면 **검증용 테스트를 추가·보강** | `pass`/`reject` + 누락 케이스 목록 + 추가한 테스트 |
...
- **하네스 미사용은 reject 사유다**: 레포에 기존 테스트 하네스·픽스처가 있는데 산출 테스트가 이를 쓰지 않았으면(예: 통합 하네스 대신 SQL 문자열 assertion) QA가 reject한다. 이 게이트의 앞단이 구현 dispatch 전의 발견 단계다 — 계약은 `delegation-patterns.md §테스트 인프라 발견`이 단일 출처. 앞단이 지켜지면 여기서 걸리는 일이 드물어야 정상이다.
...
dispatch 전에 입력 spec 문서 전부(`spec/DESIGN.md`, `spec/concerns/*`, `spec/flows/*` 또는 동등한 명세)를 read-only로 스캔한다. 다음 중 하나라도 있으면 **미결**이다:

| 신호 | 예 |
|------|-----|
| 미결 섹션에 항목이 남아 있음 | `## 미결 사항`, `## 미결정 사항`, `## TBD` 아래에 `없음`이 아닌 항목 |
| 본문의 미결 표기 | `TBD`, `미정`, `미확정`, `확정본 아님`, `추후 결정`, 답 없는 `?` 항목 |
| spec 사이의 모순 | 두 문서가 같은 계약을 다르게 서술하고 어느 쪽이 맞는지 정해지지 않음 |

확정 = 미결 섹션이 없거나 `없음`이고, 본문에 미결 표기가 없고, 모순이 없다. 판정은 문서 텍스트로 내리며 메인의 "사소해 보인다"는 체감으로 통과시키지 않는다. 결과(스캔 경로 + 미결 목록 또는 0)를 자율 계약의 `spec_resolved`와 decision log에 남긴다.
...
- [ ] spec 입력 런이면 입력 spec 전체를 미결 스캔해 **미결 0**을 확인하고 `spec_resolved`에 기록했는가? (§spec 확정 게이트 — 미결이 하나라도 있으면 계약을 세우지 않고 hard stop, 미결 목록 + 해소 경로 보고)
