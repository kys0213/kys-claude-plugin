...
- epic 브랜치에 열린 PR이 있으면 `done_when`에 "원격 최신화 완료(ahead 0)"를 포함한다 — 판정 명령은 `git` skill `SKILL.md` §열린 PR 최신화 원칙이 단일 출처다.
...
| 판정 | 함께 남기는 것 |
|------|----------------|
| `DONE` | 검증 증거 — 테스트·빌드·lint 결과, 커밋/머지 링크, (열린 PR이 있으면) push된 HEAD sha — 원격 반영 여부 확인용 (`git` skill `SKILL.md` §열린 PR 최신화 원칙) |
...
종료 시:   종료 사유 (완료 / 예산 소진 / 에스컬레이션)
           ...
           + 원격 최신화 상태 (push 완료 / PR 없음 / 위임) — `git` skill `SKILL.md` §열린 PR 최신화 원칙
...
- [ ] 현재 브랜치에 열린 PR이 있으면 push까지 완료했는가? 거부됐다면 명령을 사용자에게 위임하고 그 사실을 보고했는가? (`git` skill `SKILL.md` §열린 PR 최신화 원칙)
