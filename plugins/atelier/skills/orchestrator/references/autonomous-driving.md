...
**자율 모드로 도는 런**의 계약 수립·정지·종료 국면을 소유한다. 자율 주행 여부(모드 판정)와 HITL 모드 규칙은 → D45 가 소유하며 여기에 없다. 항목 하나는 `## D##.` / `## C##.` / `## P##.` 블록 하나이고, 그 블록이 그 항목의 유일한 소유처다.
...
```
**종단별 행동 계약**
- 해당 없음 → `spec_resolved` 에 `해당 없음`을 명시 기록한다 (→ C04). 생략은 판정이 아니다
...
```mermaid
flowchart TD
  D58_q1{"기본값에서 벗어날 런 고유 근거가 있는가"} -->|No| D58_t1["표의 기본값을 그대로 계약에 고정"]
  D58_q1 -->|Yes| D58_q2{"조정값이 사용자·상위 계약 상한 안인가"}
  D58_q2 -->|Yes| D58_t2["조정값을 근거와 함께 계약에 고정"]
  D58_q2 -->|No| D58_t3["조정하지 않고 사용자에게 회부"]
```
**종단별 행동 계약**
- 기본값 고정 → 값의 출처는 → C04 한 곳이다. 다른 블록에서 같은 예산의 수치를 다시 적는 것은 금지한다 — 대신 이름만 쓰고 → C04 를 붙인다
...
**입력 신호**
- 테스트·빌드·lint 를 실제로 돌린 명령 결과와 git 상태 (메인의 체감이 아니다)
...
```mermaid
flowchart TD
  D48_q1{"done_when 이 명령으로 판정 가능한 형태인가"} -->|No| D48_t1["자율로 진입하지 않고 종료 조건부터 합의"]
  D48_q1 -->|Yes| D48_q2{"판정 명령 결과가 전부 충족인가"}
...
```
**종단별 행동 계약**
- 종료 조건부터 합의 → 검증 가능한 `done_when` 없이 계약을 세우고 진입하는 것은 금지한다 — 대신 판정 명령까지 사용자와 합의한 뒤 진입한다 (→ C04)
...
**근거**: 매 루프 끝을 명령 결과로 재평가하지 않으면 "다 된 것 같다"가 그대로 환각 종료가 된다.
...
**입력 신호**
- 작업 단위마다 검증 증거(테스트·빌드·lint 결과, 커밋·머지 링크, 열린 PR 이면 push 된 HEAD sha — 원격 반영 여부는 `git` skill `SKILL.md` §열린 PR 최신화 원칙)가 있는가
...
| 필드 · 예산 | 값 | 도달 시 |
|---|---|---|
| `done_when` | 명령으로 판정 가능한 종료 조건 (→ D48) | 검증 불가능하면 진입하지 않는다 |
...
| `log_dir` | `.orchestrator/<epic>/decisions/` (→ C05) | — |
...
- **위치**: `log_dir` (→ C04). gitignore 되므로 커밋하지 않는다 — 자율 런의 휘발성 산출물이다. append-only `log.md` 또는 결정별 `NNNN-<slug>.md` 로 두고, epic 마다 디렉토리를 분리해 런 간 기록이 섞이지 않게 한다.
...
- **형식** (한 결정 = 한 항목): `## <ISO timestamp> · <결정 요약>` 아래 상황 / 참고 / 결정 / 근거 / 대안 / 영향. 상세 근거는 `log_dir` 과 worktree 에 남기고 메인은 경로만 보유한다 — 전문을 메인 컨텍스트로 끌어오는 것은 금지한다.
...
- **완료 시 공유**: 총 결정 수 · 주요 분기 · 전체 로그 경로를 종료 보고에 싣는다 (→ C13). 로그 자체는 커밋되지 않으므로 경로를 안내해 사용자가 직접 열 수 있게 한다.

## C06. 종료 핸드오프 기록

종료 상태는 대화가 아니라 파일로 남는다 — 다음 세션은 이번 대화를 보지 못하므로, 대화로만 보고하고 끝내면 재개하는 쪽이 상황을 처음부터 복원해야 한다. 완료·에스컬레이션·예산 소진·환경 블록 어느 쪽으로 끝나도 동일하게 남긴다.

- **위치·시점**: `.orchestrator/<epic>/HANDOFF.md` — `log_dir` (→ C04) 과 같은 디렉토리이며 gitignore 된다. 매 루프가 아니라 종료 시점에 한 번 쓴다.
- **책임 분리**: 결정 기록은 "왜 그렇게 했는가"(과정), 핸드오프는 "지금 어디이고 무엇이 막았는가"(상태)다. 한 파일에 섞지 않는다 (→ C05). **작성 주체**는 격리 worktree 로 위임한다. 메인이 직접 쓰는 것은 금지한다 — 대신 판정 목록과 근거를 prompt 에 실어 보낸다 (→ D02 · → C02).
- **내용**: 작업 단위마다 → D50 의 판정과 그 판정이 요구하는 정보(증거 / 해제 조건 / 미착수 사유). 미해결 항목과 남은 worktree 처리 상태(→ D51)도 함께 남긴다.
- **성립 기준**: 다음 세션이 이 파일만 읽고 "어디서 멈췄는지 / 무엇이 막았는지 / 무엇이 갖춰지면 재개되는지"를 알 수 있어야 한다. 대화를 봐야 이해되는 문장이 하나라도 있으면 아직 핸드오프가 아니다.
