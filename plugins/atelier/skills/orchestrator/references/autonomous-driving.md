...
**파일 단위 충돌 카운터는 `max_redispatch_per_task`와 별개다.** 예산은 task 단위라 task를 갈아치우면 리셋되지만, 같은 파일이 계속 터지는 것은 개별 작업의 실패가 아니라 **분해가 틀렸다는 신호**라 재위임으로는 풀리지 않는다. 카운트 기준과 대응은 `branch-strategy.md §충돌 반복`이 단일 출처다.

머지 **시점** 정책(배치 vs 즉시+rebase 전파)과 통합 방식(rebase 후 `--ff-only`)도 자율 모드가 따로 정하지 않는다 — `branch-strategy.md`를 그대로 따른다. 자율 모드는 in-flight가 남아 있어도 보고 없이 진행하므로, 즉시 머지를 택했다면 **전파 누락이 조용히 누적된다**는 점에서 배치 머지 기본값이 특히 중요하다.
...
- [ ] (무거운 경로) 같은 파일의 충돌 횟수를 **파일 단위로 따로** 세고 있는가? (2회 → hot-spot 재분류, 3회 → 에스컬레이션)
