...
**자율 주행 런**의 계약 수립·정지·종료 국면을 소유한다. 자율 루프 항목(D48 D58 P03)만 자율 전용. C04 는 모드 무관 값 표(HITL 도 예산·log_dir·done_when 정의를 여기서 읽는다 — 자율 계약 수립·보고만 자율 전용), C05·C06·D11·D47·D49·D50·D53 은 모드 무관이라 HITL 런도 그대로 쓴다. 모드 판정과 그 밖의 HITL 규칙은 → D45 가 소유하며 여기에 없다. 항목 하나는 `## D##.` / `## C##.` / `## P##.` 블록 하나이고, 그 블록이 그 항목의 유일한 소유처다.
...
- 경량 경로는 머지(→ D44)·게이트 verdict(→ D31) 축이 해당 없음 — 보고 수용 축만으로 판정한다
...
```
**종단별 행동 계약**
- 종료 조건부터 합의 → 자율은 판정 가능한 `done_when` 없이 계약을 세우고 진입하는 것을 금지한다 — 대신 판정 형태까지 사용자와 합의한 뒤 진입한다 (→ C04)
...
**입력 신호**
- 작업 단위마다 검증 증거(테스트·빌드·lint 결과, 커밋·머지 링크, 열린 PR 이면 push 된 HEAD sha — 원격 반영 여부는 `git` skill `SKILL.md` §열린 PR 최신화 원칙, 편집 산출이 없는 런이면 보고 수용 결과 또는 지정 경로 산출물 → D36 · → D37)가 있는가
- 응답 텍스트 보고서면 보고 수용(→ D37 · → D36) 자체가 산출물 증거다 — 파일을 요구하지 않는다
- 모드가 자율인가 HITL 인가 (→ D45) — HITL 이면 자율 루프 종료 판정(→ D48)을 거치지 않고 사용자 확인이 이 판정의 진입 신호다
...
자율 런은 진입 시 한 번 수립해 보고하고(→ C13) 그 뒤로는 계약 범위 안에서 보고 없이 진행한다. 계약을 벗어나는 순간(예산 소진 또는 hard stop)에만 멈춘다. **모든 예산·가드레일 수치의 유일한 소유처가 이 표다** — 다른 블록은 이름만 쓰고 → C04 를 붙인다.
...
- **책임 분리**: 결정 기록은 "왜 그렇게 했는가"(과정), 핸드오프는 "지금 어디이고 무엇이 막았는가"(상태)다. 한 파일에 섞지 않는다 (→ C05). **작성 주체**는 메인 또는 sub-agent 어느 쪽이든 repo 밖 경로에 쓴다 (→ D02_t3) — 격리는 필요 없다. 위임하면 판정 목록과 근거를 prompt 에 실어 보낸다 (→ C02). 경량 경로는 파일 대신 종료 보고에 같은 항목을 싣는다 (→ C13).
