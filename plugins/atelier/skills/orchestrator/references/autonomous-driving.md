...
- **spec 기반 구현이면** 일반 리뷰어 게이트 대신 `spec-driven-review.md`로 특수화한다 — 검토자(spec↔구현 적합성) + QA 매니저(spec↔테스트 적합성) 두 차원을 팀 모드로 상주시켜 worktree 코드를 계속 검증·개선한다 (예산·재위임·기록은 이 문서 규칙을 그대로 따른다).
