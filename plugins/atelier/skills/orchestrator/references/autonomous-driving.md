...
두 차원을 **서로 다른 agent**로 분리해 둔다 (한 agent가 둘 다 보면 둘 다 얕아진다):
...
- **역할 분리**: 구현·검토·QA는 **서로 다른 sub-agent**다. 같은 agent가 자기 코드를 리뷰하거나 자기 테스트만으로 통과시키지 않는다.
- **QA의 테스트 추가도 편집**이므로 `isolation:"worktree"` subagent로 위임한다 — 메인이 직접 작성하지 않는다.
- **AND 게이트**: 검토 `pass` + QA `pass`(추가한 검증 테스트 green) 둘 다여야 머지 후보로 승급한다. 하나라도 `reject`면 findings를 자기완결 prompt에 실어 재위임한다.

```
구현 완료 → 검토 에이전트 + QA 에이전트 병렬 검증     # 두 차원 동시
  둘 다 pass → 머지 후보로 승급 (이후 merge_coordinate)
  하나라도 reject → 재위임 (findings를 prompt에 포함, max_redispatch_per_task 예산 소모)
                    예산 소진 → hard stop → 에스컬레이션
```

- **병렬 검증**: 검토와 QA는 서로 독립이므로 동시에 돌린다 (한쪽이 다른 쪽을 기다리지 않음).
...
- 검토·QA 모델도 위 *모델 분배* 원칙으로 메인이 작업 리스크에 맞춰 정한다 (자동 머지의 유일한 안전장치이므로 보통 더 강한 역량을 둘 가치가 있으나, 고정은 아님).
- 컨텍스트 격리: 두 에이전트의 상세 findings·diff는 각자 컨텍스트에 남기고, 메인은 verdict + 압축 요약만 받는다.
- `done_when` 평가에 **"머지된 모든 작업이 검토 통과 + QA 검증 테스트 green"**을 포함한다.
