...
<!-- needs: D03 D08 D20 D24 -->

**입력 신호**
- 경로 판정 결과 — 무거운 경로인가 경량 경로인가 (→ D03)
- 이 작업이 되돌리기 비용이 있는 외부 쓰기를 만드는가
- 산출물이 테스트 대상인가 (→ D24 판정)
- DB 접촉 신호 — task 도출 시의 DB 접촉 플래그(→ C08) · 게이트 시점의 변경 파일 검사(마이그레이션 디렉토리 · `.sql` · 스키마 정의 · ORM 모델 · 쿼리 빌더 호출부)

```mermaid
flowchart TD
  D30_q1{"경로 판정이 무거운 경로인가"}
  D30_q1 -->|No · 경량| D30_q2{"되돌리기 비용이 있는 외부 쓰기가 있는가"}
  D30_q1 -->|Yes · 무거운| D30_q3{"산출물이 테스트 대상인가 — 코드 Yes · 문서·설정 No"}
  D30_q2 -->|Yes| D30_t1["쓰기 전 검토 1회로 축소해 실행"]
  D30_q2 -->|No| D30_t2["게이트 비발동 — 생략 판정을 기록"]
  D30_q3 -->|No| D30_t5["reviewer 1차원 게이트 — qa 생략, 사유 기록"]
  D30_q3 -->|Yes| D30_q4{"작업이 DB 에 접촉하는가"}
  D30_q4 -->|No| D30_t3["reviewer + qa 2차원 게이트를 세운다"]
  D30_q4 -->|Yes| D30_t4["reviewer + qa + dba 3차원 게이트를 세운다"]
```

**종단별 행동 계약**
- 축소 검토 1회 → 쓰기 직전 한 번만 돌고 반복 루프를 세우지 않는다. verdict 합성은 → D31
- 비발동 → 판정된 생략이므로 근거와 함께 decision log 에 남긴다 (→ C05). 보고 수용 판정은 그대로 돈다 (→ D36)
- reviewer 1차원 → 산출물이 테스트 대상이 아니므로(문서·설정 등, → D24) qa 차원을 생략하고 reviewer 만 세운다 — 생략 사유를 근거와 함께 decision log 에 남긴다 (→ C05). 구현 agent 겸임 금지는 이 종단에도 같게 적용한다 (→ D30_t3 과 동일)
