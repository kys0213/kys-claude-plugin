...
flowchart TD
  ...
  D01_q2 -->|Yes| D01_t1
  D01_q2 -->|No| D01_q3{"위임·팀·worktree·자율 주행을 사용자가 명시했는가"}
  D01_q3 -->|Yes| D01_n1(["→ D02"])
  D01_q3 -->|No| D01_q4{"여러 단위로 쪼개지거나 메인 컨텍스트를 크게 먹는가"}
  D01_q4 -->|Yes| D01_n1
  D01_q4 -->|No| D01_t2["단일 파일 단순 편집으로 분류하고 열지 않음"]
```
...
**근거**: 적용 범위는 작업의 종류가 아니라 규모로 갈리고 사용자가 명시한 위임 요구는 규모 판정보다 앞선다 — 구현 · 문서 · 조사로 가르면 read-only 리서치가 부당하게 빠진다.
...
- 통합 검증 · 머지 · 복구 · 정리 명령이거나 repo 밖 기록인가 (→ D49 · → C11 · → P09 · → C05 · → C06)

```mermaid
flowchart TD
  D02_q1{"통합 검증 · 머지 · 복구 · 정리 명령이거나 repo 밖 기록인가"} -->|Yes| D02_t3["통합 · 머지 · 복구 · 정리 명령과 repo 밖 기록(→ C05 · → C06) 작성은 메인이 직접 실행"]
  D02_q1 -->|No| D02_q2{"tracked 파일을 바꾸는 행위인가"}
  D02_q2 -->|Yes| D02_t1["위임 — 압축 요약과 verdict 만 수령"]
  D02_q2 -->|No| D02_q3{"본격 조사·통독인가"}
  D02_q3 -->|Yes| D02_t1
  D02_q3 -->|No| D02_q4{"조율 판단에 필요한 결정적 사실 확인인가"}
  D02_q4 -->|Yes| D02_t2["메인 직접 수행"]
  D02_q4 -->|No| D02_t1
```
...
- 메인 직접 수행 → 확인 범위를 조율 판단에 필요한 사실로 한정하고, 코드 본문 통독으로 번지는 순간 위임으로 되돌린다. 진입 시점 브랜치를 벗어나 다른 브랜치 · worktree 로 옮겨 가는 것은 금지한다 — 대신 격리는 위임 쪽에만 준다 (→ D20 · → D22). 메인이 직접 실행하는 것은 → D02_t3 이 열거한 것뿐이다
- 통합 · 머지 · 복구 · 정리 · repo 밖 기록 직접 실행 → 통합 검증 (→ D49) · 머지 (→ C11) · 복구 · 정리 (→ P09) 절차대로만 실행하고, repo 밖 기록(→ C05 · → C06) 작성은 tracked 편집이 아니므로 메인이 직접 남긴다. 그 밖의 편집은 여전히 위임한다
...
| D45 판정 직후 — 모드 무관하게 연다 | `references/autonomous-driving.md` | 자율은 전체(진입은 C04) · HITL 은 D48 D58 P03 제외 전부 |
...
```mermaid
flowchart TD
  P02_s0["0 · 진입 — 스키마 확보 · 경로 · 모드 판정 → P01"] --> P02_s1["1 · 분해 → D12 · 조사 런은 → D15"]
  P02_s1 --> P02_s2["2 · 충돌 위험 사전 분석 — 무거운 경로만 → P10"]
...
| 조사 · 리서치 보고 수용 전 (증거) | D37 | `investigation.md` |
| 모든 보고 수용 전 (중복) | D36 | `agent-monitor.md` |
| 매 dispatch 직후 · 완료 알림 직후 (무거운 경로) **(hard stop)** | **D21 D22** | `worktree-lifecycle.md` |
