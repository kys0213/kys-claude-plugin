...
- **파일 생성**: atelier setup 의 `orchestrator` 모듈이 주석 처리된 템플릿을 배치한다. 배치만으로는 아무 역할도 선언되지 않아 동작이 바뀌지 않으며, 주석을 푼 역할에만 정책이 적용된다.
- **스키마**:
...
- **가용성 조합별 보고** (자문은 켜는 방법을 모르면 존재하지 않는 기능과 같으므로, 침묵은 두 조합에서만):

  | 플래그 | `advisor` 선언 | 진입 보고 |
  |---|---|---|
  | on | 선언 | 자문 가용 — 계약에 `max_advisory_consults`와 함께 1회 명시 |
  | off | 선언 | "정책에 선언돼 있으나 플래그가 꺼져 있어 자문 경로를 비활성화합니다" |
  | on | 미선언 | "자문 경로를 쓰려면 정책에 `advisor.allow_models`를 선언하세요" — **1회만** 안내하고 반복하지 않는다 |
  | off | 미선언 | 침묵 (원래 없던 기능) |
