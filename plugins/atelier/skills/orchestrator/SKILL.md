...
- **사용 조건**: 진입 시 확정한 team 가용 판정(위 §진입 시 체크 4). 자문은 **team 필수 등급**이다 — 자문의 본질이 권고→반문→재답변의 왕복이라 직전 라운드를 기억하지 못하는 단발 subagent로는 **대체가 불가능**하고, advisor는 편집하지 않는 read-only 역할이라 강제 비용도 없기 때문이다(위 §team mode 강제 등급). 비가용이면 자문 경로는 사용 불가이며, 자문 없이 원래 에스컬레이션으로 진행한다. 가용인데 단발 subagent로 대체하는 것은 **위반**이다.
