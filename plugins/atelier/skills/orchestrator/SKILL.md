...
- **집행 위임(자문 제외 전부 — 구현·문서·리서치·조사·리뷰·게이트)의 tier ≤ 메인 tier, 예외 없음.** 매 dispatch에 `model` 명시 필수(상속 금지), 문서에 모델명을 박지 않는다.
- **자문 조회만 상위 tier 허용** — 권고 + 근거(read-only)만 사오고 결정권은 메인에 100% 잔류, team member 전용(필수 등급).
