...
| # | 금지 | 대신 |
|---|---|---|
| 1 | 메인이 tracked 파일을 직접 편집 | 위임한다. 예외는 통합 검증·머지·복구·정리 명령, repo 밖 기록, 그리고 이 런의 plan 기록뿐 — `plans/YYYY-MM-DD-<slug>.md` 한 파일을 구현 dispatch 이전에 쓰고 구현과 분리된 `docs` 커밋으로 남기는 데까지다. 그 밖의 문서 산출물(spec·리포트·정리 문서)은 예외가 아니라 위임 |
...
| 21 | dispatch 에 `model` 미지정(상속), 집행 위임을 메인보다 상위 tier 로, 자문 tier 를 다른 역할로 확장 | 매 dispatch `§기본값 표` 표 1 의 시작 tier 를 읽어 `model` 을 명시. 집행 위임은 표 1 사전 기준의 상한 이하, 상위는 자문 소집만. 표를 벗어나면 근거를 기록 |

## 정지 조건
신호가 관측되면 예산이 남아 있어도 멈춘다. 정지 보고는 `현재 상태 + 남은 작업 + 막힌 지점 + 선택지` 를 한 번에 담고, 종료로 이어지면 3분류 판정과 핸드오프를 남긴다. 사용자는 위임된 대화를 읽을 수 없으므로, 선택을 요청할 때는 그 선택의 근거를 질문 안에 재구성해 싣는다 (`references/contracts.md §사용자 보고 형식`).
| # | 조건 | 신호 | 행동 | 출구 |
|---|---|---|---|---|
...
2. 기록      승인 이벤트 뒤 메인이 plan 을 커밋하고 승인 마커를 세운 다음 task 를 도출한다 — 구현 dispatch 의 전제다
3. 계획      병렬 · 순차, 위임 형태, 격리 여부, 리뷰 관점을 기본값 표로 정한다
4. 위임      한 건씩, 매 건 표 1 의 tier 로 `model` 명시 — 격리 dispatch 는 생성 확인 전에 다음 건을 싣지 않는다
5. 모니터    기대 완료 시간을 정해 두고 도착하는 완료 알림으로 진행한다
6. 게이트    세운 모든 관점이 pass 여야 승급, 관점마다 다른 agent
7. 머지      한 후보씩 rebase 후 fast-forward 전용 머지, 매 머지 직후 가드
8. 최종      epic 최종 HEAD integration_verify 계약 명령 green(tracked 변경 없으면 해당 없음) + HEAD sha 명시
9. 보고      종료 사유 · 3분류 건수 · 핸드오프 · 머지 결과 · 결정 요약
```
...
### 표 1 — 집행 tier
모델 배정 기준의 canonical 은 이 표다 — 모델은 매 dispatch 마다 명시하고(상속 금지), 작업 유형에는 모델명이 아니라 역량 수준을 적는다. 집행 위임(자문을 제외한 전부 — 구현 · 문서 · 리서치 · 리뷰 · 게이트)의 tier 는 아래 사전 기준의 상한 이하이며 예외가 없다. 모델명이 나오는 곳은 이 사전 기준 한 곳뿐이다 — 세대가 바뀌면 이 표만 고친다.

사전 기준 — 메인 모델 → 집행 위임 상한. 사다리는 Fable > Opus > Sonnet > Haiku 이고, 최상위 = 상한, 중간 · 경량은 사다리에서 한 단 · 두 단 아래(바닥에 닿으면 바닥).

| 메인 모델 | 집행 위임 상한 (최상위) | 자문 소집 |
|---|---|---|
| Fable | Opus — Fable 은 집행 위임에 쓰지 않는다 | Fable (상위가 없으므로 같은 tier) |
| Opus | Opus | Fable |
| Sonnet 이하 | 메인과 같은 모델 | 메인보다 한 단 위 |
...
| 자문 소집 | 메인보다 상위(사전 기준의 자문 열) — 집행 위임 tier 상한의 유일한 예외 |
...
| 신호 | 형태 |
|---|---|
| 맥락 전달이 한 번으로 끝남 — 지시 하나에 결과 하나. 편집 dispatch 는 재위임이어도 매번 새 격리 subagent 라 여기 속한다 | 단발 subagent — 병렬 fan-out 도 단발 여러 개다 |
| 맥락을 이어 가며 여러 단계를 주고받음 — 리뷰 · 검토 · 심문 · 설계 검증처럼 findings 를 내고 개선본을 다시 받는 쪽 | agent team — 지목자를 보관하고 라운드 내내 같은 teammate 가 맡는다 |
...
| 지금 무엇이 필요한가 | 파일 |
|---|---|
| 도구 확보 · 브랜치 규약 · prompt 필수 요소 · 기록 형식 · plan 기록 · 예산 값 · 보고 형식 | `references/contracts.md` |
