...
| 21 | dispatch 에 `model` 미지정(상속), 집행 위임을 메인보다 상위 tier 로, 자문 tier 를 다른 역할로 확장 | 매 dispatch `§기본값 표` 표 1 의 시작 tier 를 읽어 `model` 을 명시. 집행 위임은 메인 tier 이하, 상위는 자문 소집만. 표를 벗어나면 근거를 기록 |
...
3. 위임      한 건씩, 매 건 표 1 의 tier 로 `model` 명시 — 격리 dispatch 는 생성 확인 전에 다음 건을 싣지 않는다
...
## 기본값 표
매번 판단하지 않고 표에서 읽는다. 표를 벗어난다고 판단한 경우에만 근거를 기록한다. 수치는 `references/contracts.md §예산과 가드레일 값` 이 소유하고 여기에는 이름만 둔다.

### 표 1 — 집행 tier
모델 배정 기준의 canonical 은 이 표다 — 모델은 매 dispatch 마다 명시하고(상속 금지), 문서에는 모델명이 아니라 역량 수준을 적는다. 집행 위임(자문을 제외한 전부 — 구현 · 문서 · 리서치 · 리뷰 · 게이트)의 tier 는 메인 tier 이하이며 예외가 없다. 최상위 = 메인과 같은 tier. 역량 수준과 실제 모델명의 대응은 dispatch 시점에 메인이 정한다.
...
### 표 2 — team 등급
필수 등급에는 가드 둘이 붙는다 — 첫 spawn 후 왕복 확인 1회(`references/contracts.md §왕복 조율 가용 판정과 spawn 확인`) · 기록의 `실행 형태`·`판정 근거` 필드(`references/contracts.md §기록 위치와 형식`).

| 경로 | 등급 | 비가용이면 |
|---|---|---|
| 아키텍트 협의체 | 필수 | 폴백 금지 — 즉시 에스컬레이션 |
| 자문 소집 | 필수 | 자문을 생략하고 원래 하려던 에스컬레이션으로. 흉내 금지 |
| 검토 게이트 (세운 관점 전부) | 선호 | 단발 subagent 폴백 — 세운 관점 수는 유지 |
| 그 밖의 왕복 위임 | 선호 | 단발 폴백 허용 |

### 표 3 — 병렬 · 순차
겹치는 파일 집합은 메인이 epic 브랜치에서 직접 만든다. 이득은 시간이고 비용은 사람이 푸는 머지 충돌이라 손해가 비대칭 — 의심스러우면 순차.
...
| overlap 이 내용 충돌 | 순차 |
| read-only 조사 (충돌 축에 걸리는 것이 없음) | 병렬 기본 — 관점이 하나뿐이라는 근거가 없으면 복수 관점 |
| 1턴에 닫히는 단발 조회 | fan-out 하지 않는다 — 메인이 그 자리에서 답한다 |
| 경량 경로 | 충돌 축이 파일 집합이 아니라 외부 리소스 · rate limit |
| 격리 불가 (편집 대상이 repo 밖) | 순차로 낮추고 보고 |

### 표 4 — 위임 형태
위임 계층은 오케스트레이터 → 작업 agent 한 단계이고, 예외는 아래 3행뿐이며 `references/contracts.md §prompt 필수 포함 요소` 8번이 이를 강제한다.

| 신호 | 형태 |
|---|---|
| 결과가 단일이고 단발 (검토 게이트는 제외 — 표 2 등급에 따름) | 단발 subagent — 병렬 fan-out 도 단발 여러 개다 |
...
### 표 6 — 검토 게이트
관점 예시 — reviewer(요구사항↔구현 · 초과 구현 · 회귀 · 설계 원칙) · qa(요구사항 · flow · 엣지케이스↔검증 테스트 · 빈 assert · 기존 하네스 사용) · dba(DB 접촉 시 스키마 변경의 하위 호환 · 락 · 롤백 · 인덱스 · N+1 · 제약조건 정합성) · 그 밖에 보안 · 성능 · 접근성 · API 호환성 등 작업 성격이 요구하는 관점. 어떤 관점을 세울지는 메인이 분해 시점에 정해 기록한다.
...
| 문서 · 설정 (tracked 편집이지만 테스트 대상 아님 — 행 1 보다 우선) | 요구사항↔산출물 관점 1개 이상 (테스트 관점 생략, 사유 기록) | 요구사항 |
...
| 축 | 판정 |
|---|---|
| 우선순위 왼쪽부터 — 단계별 확인 명시 / 종료 조건 판정 가능 / 판정 불가 | HITL / 자율(기본값) / 진입 보류 — 합의 전까지 HITL 규칙 |
| 산출물이 파일(tracked 변경 · 문서 파일 · 외부 시스템 쓰기)이면 / 응답 텍스트뿐이면 | Task 등록 / 미등록 |
| 계획된 산출물에 tracked 변경 있음 / 없음 / repo 밖 편집 | 무거운 경로 / 경량 경로 / 격리 불가 · 순차로 낮춤 · 보고 |

## 계약과 절차
| 지금 무엇이 필요한가 | 파일 |
|---|---|
| 도구 확보 · 브랜치 규약 · prompt 필수 요소 · 기록 형식 · 예산 값 · 보고 형식 | `references/contracts.md` |
