...
- 통합 검증 · 머지 · 복구 · 정리 명령인가 (→ D49 · → C11 · → P09)

```mermaid
flowchart TD
  D02_q1{"통합 검증 · 머지 · 복구 · 정리 명령인가"} -->|Yes| D02_t3["통합 · 머지 · 복구 · 정리 명령은 메인이 직접 실행"]
...
- 메인 직접 수행 → 확인 범위를 조율 판단에 필요한 사실로 한정하고, 코드 본문 통독으로 번지는 순간 위임으로 되돌린다. 진입 시점 브랜치를 벗어나 다른 브랜치 · worktree 로 옮겨 가는 것은 금지한다 — 대신 격리는 위임 쪽에만 준다 (→ D20 · → D22). 메인이 직접 실행하는 명령은 통합 검증 · 머지 · 복구 · 정리 뿐이다 (→ D02_t3)
- 통합 · 머지 · 복구 · 정리 직접 실행 → 통합 검증 (→ D49) · 머지 (→ C11) · 복구 · 정리 (→ P09) 절차대로만 실행하고, 그 밖의 편집은 여전히 위임한다
...
| D45 판정 직후 — 모드 무관하게 연다 | `references/autonomous-driving.md` | 자율은 전체(진입은 C04) · HITL 은 C05 C06 D47 D11 만 |
