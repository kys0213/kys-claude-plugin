...
- **집행 위임 = 자문 조회를 제외한 모든 위임이다.** 구현·문서 생성뿐 아니라 **리서치·조사·분석·리뷰·게이트·충돌 해결까지 전부** 포함한다. 산출물이 파일이냐 요약이냐로 가르지 않는다 — 아래 §자문 조회의 네 트리거에 해당하지 않으면 그것은 집행 위임이다.
- **집행 위임 dispatch의 tier는 메인 자신의 tier를 넘지 않는다.** (같은 원칙을 메인 관점에서 쓰면: 메인은 집행 위임되는 어떤 sub-agent보다 낮은 역량 tier로 내려가지 않는다.) 분해·위임·조율·머지 판단이 스웜 전체 결과의 상한을 결정하기 때문이다. **상위 tier가 허용되는 곳은 아래 §자문 조회뿐이다** — 조사·discovery가 아무리 중요해 보여도 집행 위임이므로 이 상한을 넘지 못한다.
...
- **사용 조건**: 진입 시 확정한 team 가용 판정(위 §진입 시 체크 4). 자문은 **team 필수 등급**이다 — 자문의 본질이 권고→반문→재답변의 왕복이라 직전 라운드를 기억하지 못하는 단발 subagent로는 **대체가 불가능**하고, advisor는 편집하지 않는 read-only 역할이라 강제 비용도 없기 때문이다(위 §team mode 강제 등급). 가용인데 단발 subagent로 대체하는 것은 **위반**이다.

- **team 비가용 = 자문 경로 차단** (이 규칙은 `references/advisory-consult.md`를 읽지 않아도 성립해야 하므로 본문에 둔다). 비가용이 확정된 세션에서 메인이 하는 것은 셋뿐이다:
  1. **소집하지 않는다** — 자문 트리거에 도달해도 `Agent({name: "advisor-..."})`를 호출하지 않는다. 가용성은 트리거의 부속 조건이 아니라 **선행 게이트**다.
  2. **대체하지 않는다** — 단발 subagent 1회 왕복도, 메인 자신의 상위 tier 흉내도 자문이 아니다 (아래 안티패턴 8).
  3. **원래 하려던 에스컬레이션으로 진행하고 기록한다** — decision log에 "자문 경로 비활성(판정 근거)으로 소집 없이 에스컬레이션"을 남긴다. 자문이 없었다는 사실 자체가 사후 검토 대상이다.
  - **사용자가 자문을 명시 요청해도 우회하지 못한다** — 경로가 없으면 흉내 내지 말고 자문이 불가능한 상태임을 보고한다. 사용자가 원한 것은 상위 tier의 관점이지 "자문했다는 서술"이 아니다.
  - 자율 모드에서는 계약의 `max_advisory_consults`가 **0**으로 고정된다 (`references/autonomous-driving.md §자율 계약`) — 예산이 0이면 트리거 지점에서 쓸 수 있는 것이 없다.
