...
| `references/stagnation-redirect.md` | task 단위 정체 방어 (simhash/Jaccard → persona 재설정). **현재 비활성(deferred)** — PreToolUse 훅(`protect-stagnation.sh`)이 `task claim --epic` 경로에서 구조적으로 못 떠 #776 에서 제거됨. `autopilot check stagnation` CLI core 는 유지, `task claim` 내부 통합으로 재활성 예정 | (deferred) |

> **정체 방어**: `orchestrator` skill 의 `references/autonomous-driving.md` 가 **루프 단위**(분해→위임→머지 self-drive 의 no_progress/예산) 방어를 담당합니다. **task 단위** 방어(`stagnation-redirect.md`)는 PreToolUse 훅 제거(#776)로 현재 비활성이며 `task claim` 내부 통합으로 재활성 예정입니다.
