# PR 머지 파이프라인 (merge-prs)

autopilot 이 만든 PR(`{label_prefix}auto`)을 분류 → all-green 즉시 머지 → 문제 PR 해결까지 처리. 전처리(base 동기화·idle/capacity·throttling)는 `pipeline-control.md`.

> **머지·worktree·병렬 dispatch 메커니즘은 `orchestrator` skill 에 위임**한다 (단일 소유, SKILL.md §"머지·병렬·worktree"). 본 reference 는 "무엇을 위임할지(입력)"와 머지 성공/실패 시의 결정적 후속 처리(ledger close-the-loop, `Closes #N`, worktree cleanup 가드)만 정의한다. 병렬 pr-merger 디스패치·worktree 정리의 *방법*은 orchestrator 의 `references/merge-coordinator.md`·`worktree-lifecycle.md`·`delegation-patterns.md` 가 단일 소유한다.

> 인자: `--branch=<branch>` (특정 브랜치 PR 만 대상). hybrid 모드에서 이벤트로 전달. 있으면 Step 2 에서 해당 브랜치만 조회: `gh pr list --head "{branch}" --label "{label_prefix}auto" --state open --json ... --limit 1`.

전처리(`pipeline-control.md`)는 capacity 검사 불필요 — `--max-parallel` 없이 idle/active 2값으로 호출, loop 이름 `merge-prs`.
...
PR 이 없으면 `autopilot check mark merge-prs --status idle` 후 "머지 대상 PR 없음" 출력 후 종료.
...
> CI 실패 PR 은 ci-fix 루프가 tick 단위로 수정한다. merge-prs 는 CI 실패를 직접 수정하지 않는다.

### Step 4: All-green PR 즉시 머지

머지 전 idle count 리셋: `autopilot check mark merge-prs --status active`.

**중요**: `gh pr merge` 의 종료 코드를 반드시 검사한다. 동시 머지 경합(`Base branch was modified` / `Pull Request is not mergeable`)으로 머지가 실패하면 ledger close, `Closes #N` 이슈 close, worktree 정리는 **실행되지 않아야** 하며, 해당 PR 은 Step 5(문제 PR 분류)로 라우팅한다.
...
> 이 단계의 어떤 실패도 fast-path 머지 결과를 변경하지 않는다. 동일 로직이 `agents/pr-merger.md` Step 4 에도 존재한다 (문제 PR 경로용).

**머지 종료 코드 가드 (#643):** 위 `if gh pr merge ...; then ... else ... fi` 구조는 머지 실패 시 `Closes #N` 이슈가 잘못 close 되는 것을 방지한다. 실패 시 PR 은 Step 5 의 `pr-merger` 에이전트에게 위임되어 conflict / changes-requested 분류 후 처리된다.

### Step 5: 문제 PR 해결 (Agent Team — orchestrator 위임)

문제가 있는 PR 각각에 대해 pr-merger 에이전트**를** 디스패치한다. **PR 3개 이하**: 순차(background=false). **PR 4개 이상**: 병렬(background=true). 병렬 dispatch·worktree·머지 조정 메커니즘은 `orchestrator` skill 에 위임한다.

각 에이전트에 전달: pr_number, pr_title, problems(감지된 문제 목록 — conflict, review_changes_requested).

> CI 실패는 ci-fix 루프에서 처리하므로 pr-merger 에 전달하지 않는다.

**중요**: 사람이 명시적으로 `CHANGES_REQUESTED` 리뷰를 남긴 PR 은 자동 머지하지 않는다. pr-merger 가 코멘트에 응답만 하고, 머지는 사람의 재리뷰 후 다음 cycle 에서 처리한다.
...
매 cycle 종료 시 세션 통계 업데이트:

- `PROCESSED` = Step 2 에서 평가 대상이 된 PR 수 (all-green + 문제 PR 합)
- `SUCCESS` = Step 4 fast-path 또는 Step 5 pr-merger 경유로 머지 완료된 PR 수
- `FAILED` = 머지 시도/문제 해결이 실패한 PR 수 (conflict 미해결, pr-merger fail 등)
- `FALSE_POSITIVE` = `0` (merge-prs 는 false-positive 분류 단계 없음 — 사람 CHANGES_REQUESTED 보류는 success/failed 어느 쪽도 아니며 다음 cycle 로 넘김)

```bash
autopilot stats update --command merge-prs \
