...
| 3 | `branch_strategy: "draft-main"` (또는 미설정) | `main` |

> **알 수 없는 `branch_strategy` 값 처리**: 위 enum(`draft-main`, `draft-develop-main`)에
> 없는 값(오타 등)은 안전을 위해 `main`으로 폴백하되, **silent fallback이 아니라 경고**로
> 다룬다. `atelier autopilot preflight`가 알 수 없는 `branch_strategy`를 감지하면 WARN을
> 출력하므로, 설정 직후 `preflight`로 검증한다.
