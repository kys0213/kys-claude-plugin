---
name: git
description: ALWAYS use this skill for ANY git-related task (commit, push, branch, PR, status, diff, log, conflict resolution, unresolved review followup, issue prioritization, issue status labeling). Provides automatic quality validation and enforces project conventions. Mechanical calls (guard/hook/reviews) go through the atelier git CLI; commit/branch/PR run as plain git/gh applying the conventions below.
version: 1.0.0
allowed-tools: Bash
---
...
현재 프로젝트의 **형상관리 워크플로우**를 자동화합니다. 브랜치 생성, TODO별 커밋, push, PR 생성, **rebase conflict 해결**, 미해결 리뷰 정리, 이슈 우선순위 추천, **이슈 기반 작업 착수 시 상태 라벨 표시**를 프로젝트 컨벤션에 맞게 처리합니다.
...
- "이 이슈 작업할게", "이슈 잡고 시작할게" (이슈 기반 작업 착수 — 상태 라벨 표시)
...
| `references/issue-workflow.md` | 이슈 기반 작업 착수 시 진행 상태를 이슈 라벨(`status: in progress`)로 표시하는 규칙·전이·안티패턴 | "이 이슈 작업할게" / 이슈 잡고 시작 |
