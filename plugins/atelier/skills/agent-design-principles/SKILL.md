...
특정 모델명을 작업 유형에 고정 매핑하지 않는다. 대신 **작업의 난이도·리스크·되돌리기 비용**에 역량을 맞추고, dispatch 시점마다 재평가한다 — 되돌리기 쉬운 단순 작업엔 가벼운 모델을, 판단 오류의 대가가 큰 작업(아키텍처 설계, 비가역 변경)엔 강한 모델을 배정한다. 구체적인 배정 기준과 재평가 절차는 `orchestrator` `references/contracts.md §기본값 표` 의 집행 tier 표가 canonical 이다.
...
- **user-invocable Skill** (`skills/`, `user-invocable: true`): User + 모델 맥락 자동 발동. 도메인이 맞으면 모델이 알아서 꺼내써야 하는 것. 예: git, grill, spec-write, communicate, workflow, orchestrator.
...
판단이 필요 없는 작업(예: 훅 등록, 설정 편집)은 LLM이 매번 구조를 해석하며 편집하게 두지 말고 CLI/스크립트로 캡슐화해 LLM은 결과만 확인하게 합니다.

#### 전략 3: CLAUDE.md는 정적 원칙만

기술 스택·빌드·테스트·컨벤션처럼 잘 안 변하는 정적 정보만 CLAUDE.md에 담습니다. 현재 이슈 번호나 오늘의 우선순위처럼 동적으로 자주 바뀌는 정보는 대화나 Sub-agent Context로 전달합니다.

---

## 안티패턴 목록

- **Fat Controller (뚱뚱한 Command)**: Command에 로직이 직접 들어있음 → Sub-agent로 위임하고 Command는 진입점만 유지
- **God Agent (만능 Agent)**: 하나의 Agent가 모든 것을 담당 → 역할별로 Agent 분리, 적절한 model 선택
- **Skill Explosion (스킬 폭발)**: 과도하게 잘게 쪼갠 Skill → 관련된 것은 하나의 Skill로 통합
- **Chatty Context (수다스러운 컨텍스트)**: CLAUDE.md에 동적 정보까지 담으려 함 → 정적 원칙만 CLAUDE.md에, 나머지는 대화로
- **LLM에게 결정적 로직 위임**: 판단이 필요 없는 로직을 매번 LLM이 해석 → 결정적 판단은 CLI로 캡슐화

---

## HITL (Human-in-the-Loop)

모든 것을 미리 정의하려 하기보다는, git/package.json 등으로 환경을 자동 감지하되 애매한 순간에만 질문하는 접근을 고려하세요.
