//! HTTP delivery abstraction. The trait keeps the command unit-testable; the
//! real implementation shells out to `curl` so the binary carries no HTTP
//! client dependency. Missing `curl` surfaces as a per-channel error in the
//! report — never a non-zero exit.
