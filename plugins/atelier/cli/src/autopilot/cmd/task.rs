...
use crate::autopilot::cmd::output::write_json;
use crate::autopilot::cmd::simhash;
use crate::autopilot::domain::{
    DomainError, Task, TaskFailureOutcome, TaskId, TaskSource, TaskStatus,
};
use crate::autopilot::ports::clock::{Clock, StdClock};
use crate::autopilot::ports::task_store::{NewWatchTask, TaskStore, TaskStoreError, UpsertOutcome};
...
impl<'a> TaskService<'a> {
    ...
    pub fn add(
        ...
        let id = TaskId::parse(task_id).map_err(|e| {
            anyhow::Error::new(crate::autopilot::domain::UserInputError::new(e.to_string()))
        })?;
        let now = self.clock.now();
        let fp = fingerprint
            .map(str::to_string)
    ...
    pub fn add_batch(&self, epic: &str, path: &Path, out: &mut dyn Write) -> Result<i32> {
        ...
        for (lineno, line) in reader.lines().enumerate() {
            ...
            let id = TaskId::parse(&parsed.id).map_err(|e| {
                anyhow::Error::new(crate::autopilot::domain::UserInputError::new(format!(
                    "{} (line {})",
                    e,
                    lineno + 1
                )))
...
fn derive_task_simhash(simhash_input: Option<&str>, title: &str, body: Option<&str>) -> u64 {
    use crate::autopilot::domain::simhash::derive_simhash;
    match simhash_input {
...
mod tests {
    use super::*;
    use crate::autopilot::ports::clock::FixedClock;
    use crate::autopilot::ports::task_store::{EpicPlan, NewTask};
    use crate::autopilot::store::InMemoryTaskStore;
    use chrono::TimeZone;
    ...
    fn fixture() -> (Arc<dyn TaskStore>, FixedClock) {
        ...
        store
            .insert_epic_with_tasks(
                EpicPlan {
                    epic: crate::autopilot::domain::Epic {
                        name: "e".to_string(),
                        spec_path: PathBuf::from("spec/x.md"),
                        branch: "epic/e".to_string(),
                        status: crate::autopilot::domain::EpicStatus::Active,
                        created_at: now,
                        completed_at: None,
                    },
                    tasks: vec![NewTask {
                        id: TaskId::from_raw("a1b2c3d4e5f6"),
                        source: crate::autopilot::domain::TaskSource::Decompose,
                        fingerprint: None,
