...
fn check_guard_hook(fs: &dyn FsOps, repo_root: &Path, results: &mut Vec<CheckResult>) {
    // setup은 user scope(~/.claude/settings.json)에 hook을 설치하지만 프로젝트 단위로
    // 직접 등록하는 경우도 있으므로 두 scope를 모두 확인한다. (project scope만 보면
    // 정상 설치 후에도 항상 WARN이 떠서 잘못된 hook을 수동으로 박는 사고로 이어진다 — #731)
    ...
    if any_settings_found {
        results.push(CheckResult::warn(
            "Hooks",
            "guard-pr-base hook not found — run /atelier:setup (do not edit settings.json manually)",
        ));
    } else {
...
/// frontmatter에서 `key: "value"` 형태의 스칼라 값을 추출한다 (따옴표/인라인 주석 제거).
...
/// branch_strategy 값이 알려진 enum인지 검증한다. 알 수 없는 값을 silent fallback to
/// main 으로 흘려보내지 않고 명시적으로 WARN을 띄운다 (#758).
