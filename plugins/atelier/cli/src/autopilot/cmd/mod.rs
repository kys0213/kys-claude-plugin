...
/// Args for the `atelier autopilot <...>` subcommand group. Embedded as a
/// subcommand of the top-level `atelier` router (see `crate::cli`); top-level
/// owns `name`/`version`, so this only declares the autopilot-scoped flags.
#[derive(Args)]
