use crate::autopilot::cmd::{
    AutopilotCli, CheckCommands, Commands, IssueCommands, ListArgs, PipelineCommands,
...
/// Runs the `atelier autopilot <...>` subcommand tree. Mirrors the original
/// standalone `autopilot` binary entry point; top-level argument parsing and
/// logger init happen in the atelier binary, so this takes an already-parsed
/// [`AutopilotCli`] and dispatches it.
pub fn run(cli: AutopilotCli) {
