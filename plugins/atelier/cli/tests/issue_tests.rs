...
fn default_create_args() -> atelier::autopilot::cmd::issue::CreateArgs {
    atelier::autopilot::cmd::issue::CreateArgs {
...
fn check_dup_returns_1_when_duplicate_exists() {
    ...
    let code =
        atelier::autopilot::cmd::issue::check_dup(&gh, "gap:spec/auth.md:token-refresh").unwrap();
    assert_eq!(code, 1);
}
...
fn create_skips_when_duplicate_exists() {
    ...
    let code = atelier::autopilot::cmd::issue::create(&gh, &args).unwrap();
    assert_eq!(code, 1);
...
fn close_resolved_closes_merged_branch_issues() {
    ...
    let code = atelier::autopilot::cmd::issue::close_resolved(&gh, "autopilot:").unwrap();
    assert_eq!(code, 0);
...
fn create_detects_duplicate_from_previous_tick() {
    ...
    let args = atelier::autopilot::cmd::issue::CreateArgs {
    ...
    let code = atelier::autopilot::cmd::issue::create(&gh_tick1, &args).unwrap();
    ...
    let code = atelier::autopilot::cmd::issue::create(&gh_tick2, &args).unwrap();
...
fn check_dup_handles_fingerprint_with_colons_and_slashes() {
    ...
    let code =
        atelier::autopilot::cmd::issue::check_dup(&gh, "ci:validate.yml:fix/CP-920:build-error")
            .unwrap();
...
fn create_passes_multiple_labels_correctly() {
    ...
    atelier::autopilot::cmd::issue::create(&gh, &args).unwrap();
...
fn append_fingerprint_preserves_body_with_special_markdown() {
    ...
    let result = atelier::autopilot::cmd::issue::append_fingerprint(body, fp);
...
fn extract_branch_from_ci_title_various_formats() {
    // Standard format
    assert_eq!(
        atelier::autopilot::cmd::issue::extract_branch_from_ci_title(
    ...
    // No " on " → empty
    assert_eq!(
        atelier::autopilot::cmd::issue::extract_branch_from_ci_title("some random title"),
        ""
    );
    ...
    let gh = MockGh::new().on_list_containing(
        ...
        vec![
            json!({
                ...
                "title": "far issue",
                "state": "OPEN",
                "body": make_issue_body_with_simhash("content", "gap:spec:x", &atelier::autopilot::cmd::simhash::format_simhash(far))
            }),
            json!({
                "number": 11,
                "title": "close issue",
                "state": "CLOSED",
                "body": make_issue_body_with_simhash("content", "gap:spec:x", &atelier::autopilot::cmd::simhash::format_simhash(close))
            }),
        ],
    );

    let args = atelier::autopilot::cmd::issue::SearchSimilarArgs {
        fingerprint: "gap:spec:x".to_string(),
        simhash: atelier::autopilot::cmd::simhash::format_simhash(base),
        limit: 5,
    };

    let code = atelier::autopilot::cmd::issue::search_similar(&gh, &args).unwrap();
    assert_eq!(code, 0);
}
...
fn search_similar_invalid_simhash_returns_error() {
    ...
    let result = atelier::autopilot::cmd::issue::search_similar(&gh, &args);
