...
use atelier::autopilot::cmd::task::{TaskService, TaskSourceArg, TaskStatusArg};
use atelier::autopilot::domain::{Epic, EpicStatus, EventKind, TaskId, TaskSource, TaskStatus};
use atelier::autopilot::ports::clock::{Clock, FixedClock};
use atelier::autopilot::ports::task_store::{EpicPlan, EventFilter, NewTask, TaskStore};
...
fn seed_task(
    ...
    use atelier::autopilot::ports::task_store::ReconciliationPlan;
    let existing = store
        .get_epic("e")
        .unwrap()
...
fn add_with_explicit_simhash_input_and_paths_persists_them() {
    ...
    use atelier::autopilot::domain::simhash::derive_simhash;
    let (store, clock) = fixture();
    let svc = TaskService::new(store.as_ref(), &clock);
...
fn release_stale_rejects_task_id_combined_with_before() {
    use atelier::cli::Atelier;
    use clap::Parser;
    let result = Atelier::try_parse_from([
        "atelier",
        "autopilot",
        "task",
        "release-stale",
...
fn parse_duration_seconds_supports_compound_units() {
    use atelier::autopilot::cmd::task::parse_duration_seconds;
    // Single units
...
fn assert_set_status_parses(subcmd: &str) {
    use atelier::autopilot::cmd::{Commands, TaskCommands};
    use atelier::cli::{Atelier, Group};
    use clap::Parser;
    let cli = Atelier::try_parse_from([
        "atelier",
        "autopilot",
        "task",
        subcmd,
