...
impl Workspace {
    ...
    /// Build a fresh `Command` for the `atelier` binary, scoped to this
    /// workspace and pre-routed to the `autopilot` subsystem. Each call
    /// returns a new builder so callers may chain `.args(...)` without
    /// leaking state between invocations.
    ///
    /// Post Phase 2: the standalone `autopilot` binary became
    /// `atelier autopilot <...>`. This helper injects the `autopilot`
    /// subcommand so existing scenarios — which append the autopilot
    /// args directly — keep exercising the same surface.
