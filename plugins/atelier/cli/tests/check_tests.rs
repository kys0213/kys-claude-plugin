...
use atelier::autopilot::cmd::check::spec_code::SpecCodeAnalysis;
use atelier::autopilot::cmd::check::state::state_file_path;
use atelier::autopilot::cmd::check::{
...
fn mark_status_idle_increments_count() {
    ...
    make_svc(git, fs.clone())
        .mark(
            "build-issues",
            None,
            Some(&atelier::autopilot::cmd::LoopStatus::Idle),
        )
        .unwrap();
    let written = fs.written_files();
    let state: serde_json::Value = serde_json::from_str(&written[0].1).unwrap();
...
fn mark_status_active_resets_count() {
    ...
    make_svc(git, fs.clone())
        .mark(
            "build-issues",
            None,
            Some(&atelier::autopilot::cmd::LoopStatus::Active),
        )
        .unwrap();
    let written = fs.written_files();
    let state: serde_json::Value = serde_json::from_str(&written[0].1).unwrap();
