...
fn body_reconcile_is_idempotent(store: Arc<dyn TaskStore>) {
    ...
    let key = |t: &atelier::autopilot::domain::Task| {
        (
            t.id.clone(),
...
fn body_force_status_records_event_with_reason(store: Arc<dyn TaskStore>) {
    ...
    assert_eq!(evs.len(), 1);
    match &evs[0].payload {
        atelier::autopilot::domain::EventPayload::TaskForceStatus { reason, to, .. } => {
...
macro_rules! conformance_suite {
    ($($name:ident),* $(,)?) => {
        mod in_memory {
            use std::sync::Arc;
            use atelier::autopilot::ports::task_store::TaskStore;
