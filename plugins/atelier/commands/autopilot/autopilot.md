...
`spec_paths`에 스펙 파일이 있으면, `gap-auditor` 에이전트를 호출하여 스펙 품질을 평가합니다:
...
- **No**: `/atelier:design` 또는 `/atelier:spec-review` 안내 후 종료
...
1. `spec_paths`에 스펙 파일이 있으면 `/atelier:gap-watch`를 실행합니다
2. 발견된 갭이 이슈로 등록됩니다
3. 결과 로그:
...
1. CLI로 미분석 이슈를 조회합니다:
   ...
   ```
2. 반환된 이슈들에 대해 `/atelier:analyze-issue {number}`를 병렬 실행합니다 (`max_parallel_agents` 단위로 분할)
4. 결과 로그:
...
| 이벤트 | 조건 | 액션 |
|--------|------|------|
| `MAIN_UPDATED before=<sha> after=<sha> count=<N>` | — | `/atelier:gap-watch` 후 `/atelier:qa-boost {before}` |
| `CI_FAILURE run_id=<id> workflow=<name> branch=<branch>` | default branch | `/atelier:ci-watch --run-id={run_id} --branch={branch}` |
| `CI_FAILURE run_id=<id> workflow=<name> branch=<branch>` | autopilot branch | `/atelier:ci-fix --branch={branch}` |
| `CI_SUCCESS run_id=<id> workflow=<name> branch=<branch>` | autopilot branch | `/atelier:merge-prs --branch={branch}` |
| `NEW_ISSUE number=<N> title=<title>` | — | `/atelier:analyze-issue {number}` |
| `TASK_READY epic=<E> task_id=<ID>` | — | `/atelier:work-ledger --epic {E}` (해당 epic만 claim+dispatch — Ready task가 새로 생기거나 unblock될 때마다 즉시 처리, cron 폴링 불필요) |
| `STALE_WIP candidates=<JSON> epic=<E>` | — | `/atelier:stale-task-review --candidates {JSON}` (이미 daemon이 list-stale로 필터링한 후보를 그대로 전달, skill은 list-stale 호출을 skip하고 stale-task-reviewer 에이전트로 바로 디스패치) |
| `EPIC_DONE epic=<E> total=<N>` | — | **로그만 기록** (TODO: feat-epic-flow epic에서 `/atelier:epic-rollup {E}` 디스패치 예정 — 현재는 epic-rollup 커맨드 미존재) |
...
1. `CronCreate(cron: "{build_issues_cron}", prompt: "/atelier:build-issues")` — GitHub issues를 직접 읽으므로 SQLite events에 신호가 없습니다. 폴링 유지.
2. test_watch 배열이 비어있지 않으면 각 스위트별:
   `CronCreate(cron: "{suite_interval_cron}", prompt: "/atelier:test-watch {suite.name}")` — 시간 기반 스케줄 테스트.
...
1. `CronCreate(cron: "{gap_watch_cron}", prompt: "/atelier:gap-watch")`
2. `CronCreate(cron: "{analyze_issue_cron}", prompt: "/atelier:analyze-issue")`
3. `CronCreate(cron: "{build_issues_cron}", prompt: "/atelier:build-issues")`
4. `CronCreate(cron: "{merge_prs_cron}", prompt: "/atelier:merge-prs")`
5. `CronCreate(cron: "{ci_watch_cron}", prompt: "/atelier:ci-watch")`
6. `CronCreate(cron: "{ci_fix_cron}", prompt: "/atelier:ci-fix")`
7. `CronCreate(cron: "{qa_boost_cron}", prompt: "/atelier:qa-boost")`
8. `CronCreate(cron: "{work_ledger_cron}", prompt: "/atelier:work-ledger")` — gap-backlog / qa-backlog / ci-backlog ledger reader (Step 2.5에서 스냅샷으로 보여주는 3개 backlog의 Ready task를 claim해 issue-implementer로 디스패치)
9. `CronCreate(cron: "{release_stale_cron}", prompt: "/atelier:stale-task-review --before {stale_wip.threshold}")` — stale Wip 관찰 + 에이전트 리뷰 (worker crash / ctrl-C / worktree 파괴 복구). CLI (`autopilot task list-stale`) 는 cutoff 보다 오래된 Wip 후보를 read-only 로 반환하고, `stale-task-reviewer` 에이전트가 task 별로 release / fail / escalate / leave alone 을 결정합니다. exit 0 (idempotent — 후보가 0건이어도 정상). 결정은 컨텍스트 의존이므로 CLI 가 아닌 에이전트가 수행합니다 (CLAUDE.md "책임 경계").

test_watch 배열이 비어있지 않으면 각 스위트별 루프 추가:
```
CronCreate(cron: "{suite_interval_cron}", prompt: "/atelier:test-watch {suite.name}")
```
...
|------|---------|----------|------|
| Build Issues | /atelier:build-issues | {interval} | {cron} |
| Monitor Health | health check | 30m | */30 * * * * |
| Test: {name} | /atelier:test-watch {name} | {interval} | {cron} |
...
| Loop | Command | Interval | Cron |
|------|---------|----------|------|
| Gap Watch | /atelier:gap-watch | 30m | */30 * * * * |
| Analyze Issue | /atelier:analyze-issue | 20m | */20 * * * * |
| Build Issues | /atelier:build-issues | 15m | */15 * * * * |
| Merge PRs | /atelier:merge-prs | 10m | */10 * * * * |
| CI Watch | /atelier:ci-watch | 20m | */20 * * * * |
| CI Fix | /atelier:ci-fix | 15m | */15 * * * * |
| QA Boost | /atelier:qa-boost | 1h | 7 * * * * |
| Work Ledger | /atelier:work-ledger | 10m | */10 * * * * |
| Stale Task Review | /atelier:stale-task-review --before 1h | 30m | */30 * * * * |
| Test: e2e | /atelier:test-watch e2e | 2h | 7 */2 * * * |
