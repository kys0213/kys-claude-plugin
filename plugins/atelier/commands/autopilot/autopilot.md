...
1. `spec_paths`에 스펙 파일이 있으면 autopilot skill 의 `references/gap-watch.md` 절차를 수행합니다
2. 발견된 갭이 이슈로 등록됩니다
3. 결과 로그:
...
1. CLI로 미분석 이슈를 조회합니다:
   ...
   ```
2. 반환된 이슈들에 대해 autopilot skill 의 `references/build-pipeline.md` 이슈 분석 절차를 병렬 수행합니다 (issue-analyzer 에이전트) (`max_parallel_agents` 단위로 분할)
4. 결과 로그:
...
| 이벤트 | 조건 | 액션 |
|--------|------|------|
| `MAIN_UPDATED before=<sha> after=<sha> count=<N>` | — | autopilot skill `references/gap-watch.md` 후 `references/qa-boost.md` ({before}) |
| `CI_FAILURE run_id=<id> workflow=<name> branch=<branch>` | default branch | autopilot skill `references/ci.md` §ci-watch ({run_id}, {branch}) |
| `CI_FAILURE run_id=<id> workflow=<name> branch=<branch>` | autopilot branch | autopilot skill `references/ci.md` §ci-fix ({branch}) |
| `CI_SUCCESS run_id=<id> workflow=<name> branch=<branch>` | autopilot branch | autopilot skill `references/merge.md` ({branch}) |
| `NEW_ISSUE number=<N> title=<title>` | — | autopilot skill `references/build-pipeline.md` 이슈 분석 ({number}) |
| `TASK_READY epic=<E> task_id=<ID>` | — | autopilot skill `references/ledger.md` §A work-ledger (--epic {E}, 해당 epic만 claim+dispatch) |
| `STALE_WIP candidates=<JSON> epic=<E>` | — | autopilot skill `references/ledger.md` §B stale-task-review (--candidates {JSON}, list-stale skip 후 stale-task-reviewer 디스패치) |
...
1. `CronCreate(cron: "{build_issues_cron}", prompt: "autopilot skill references/build-pipeline.md 절차 수행")` — GitHub issues를 직접 읽으므로 SQLite events에 신호가 없습니다. 폴링 유지.
...
8. `CronCreate(cron: "{work_ledger_cron}", prompt: "autopilot skill references/ledger.md §A work-ledger 수행")` — gap-backlog / qa-backlog / ci-backlog ledger reader (Step 2.5에서 스냅샷으로 보여주는 3개 backlog의 Ready task를 claim해 issue-implementer로 디스패치)
