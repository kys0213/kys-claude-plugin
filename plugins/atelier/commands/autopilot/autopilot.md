...
- Exit 1: FAIL 항목 있음 → FAIL 항목을 사용자에게 보여주고 `/atelier:autopilot/setup` 안내 후 중단
...
- **No**: `/atelier:spec/design` 또는 `/atelier:spec/spec-review` 안내 후 종료
...
1. `spec_paths`에 스펙 파일이 있으면 `/atelier:autopilot/gap-watch`를 실행합니다
2. 발견된 갭이 이슈로 등록됩니다
3. 결과 로그:
...
1. CLI로 미분석 이슈를 조회합니다:
   ...
   ```
2. 반환된 이슈들에 대해 `/atelier:autopilot/analyze-issue {number}`를 병렬 실행합니다 (`max_parallel_agents` 단위로 분할)
4. 결과 로그:
...
> `--stale-threshold`(W1, PR #701)는 daemon이 SQLite 폴링으로 stale Wip을 감지하여 `STALE_WIP` 이벤트를 emit하기 위한 cutoff입니다. `stale_wip.threshold` 설정값을 그대로 전달합니다.
...
| 이벤트 | 조건 | 액션 |
|--------|------|------|
| `MAIN_UPDATED before=<sha> after=<sha> count=<N>` | — | `/atelier:autopilot/gap-watch` 후 `/atelier:autopilot/qa-boost {before}` |
| `CI_FAILURE run_id=<id> workflow=<name> branch=<branch>` | default branch | `/atelier:autopilot/ci-watch --run-id={run_id} --branch={branch}` |
| `CI_FAILURE run_id=<id> workflow=<name> branch=<branch>` | autopilot branch | `/atelier:autopilot/ci-fix --branch={branch}` |
| `CI_SUCCESS run_id=<id> workflow=<name> branch=<branch>` | autopilot branch | `/atelier:autopilot/merge-prs --branch={branch}` |
| `NEW_ISSUE number=<N> title=<title>` | — | `/atelier:autopilot/analyze-issue {number}` |
| `TASK_READY epic=<E> task_id=<ID>` | — | `/atelier:autopilot/work-ledger --epic {E}` (해당 epic만 claim+dispatch — Ready task가 새로 생기거나 unblock될 때마다 즉시 처리, cron 폴링 불필요) |
| `STALE_WIP candidates=<JSON> epic=<E>` | — | `/atelier:autopilot/stale-task-review --candidates {JSON}` (이미 daemon이 list-stale로 필터링한 후보를 그대로 전달, skill은 list-stale 호출을 skip하고 stale-task-reviewer 에이전트로 바로 디스패치) |
| `EPIC_DONE epic=<E> total=<N>` | — | **로그만 기록** (TODO: feat-epic-flow epic에서 `/atelier:autopilot/epic-rollup {E}` 디스패치 예정 — 현재는 epic-rollup 커맨드 미존재) |

> 동시에 여러 이벤트가 도착하면 독립적인 이벤트는 병렬 디스패치합니다. 같은 브랜치에 CI_FAILURE와 CI_SUCCESS가 동시에 도착하면 최신 이벤트만 처리합니다.
>
> **Ledger 이벤트 (W1, PR #701)**: `autopilot watch`가 SQLite events 테이블을 폴링하여 emit합니다. daemon은 idempotency state(`watch.json`)를 유지하므로 동일 이벤트가 두 번 디스패치되지 않습니다 — Monitor 측에서 별도 dedup이 필요 없습니다. `STALE_WIP`의 `<JSON>`은 task id 문자열 배열(예: `["abc123","def456"]`)로 그대로 `--candidates`에 전달합니다.

#### Phase B: CronCreate 등록 (이벤트 신호가 없는 폴링 컴포넌트만)

이벤트로 신호화할 수 없는 컴포넌트만 CronCreate로 등록합니다.

> **W1 (PR #701) 이후 변경**: `work-ledger` / `stale-task-review` 의 cron 등록은 **제거되었습니다**. `autopilot watch` daemon이 ledger의 SQLite events를 폴링하여 `TASK_READY` / `STALE_WIP` 이벤트를 emit하므로, Phase A Monitor 디스패치 표가 cron을 대체합니다. 동일한 트리거를 두 경로에서 중복 등록하지 않습니다 (CLAUDE.md "책임 경계" — daemon은 emit, Monitor는 dispatch).

1. `CronCreate(cron: "{build_issues_cron}", prompt: "/atelier:autopilot/build-issues")` — GitHub issues를 직접 읽으므로 SQLite events에 신호가 없습니다. 폴링 유지.
2. test_watch 배열이 비어있지 않으면 각 스위트별:
   `CronCreate(cron: "{suite_interval_cron}", prompt: "/atelier:autopilot/test-watch {suite.name}")` — 시간 기반 스케줄 테스트.
...
기본 8개 루프를 `CronCreate`로 등록합니다:

1. `CronCreate(cron: "{gap_watch_cron}", prompt: "/atelier:autopilot/gap-watch")`
2. `CronCreate(cron: "{analyze_issue_cron}", prompt: "/atelier:autopilot/analyze-issue")`
3. `CronCreate(cron: "{build_issues_cron}", prompt: "/atelier:autopilot/build-issues")`
4. `CronCreate(cron: "{merge_prs_cron}", prompt: "/atelier:autopilot/merge-prs")`
5. `CronCreate(cron: "{ci_watch_cron}", prompt: "/atelier:autopilot/ci-watch")`
6. `CronCreate(cron: "{ci_fix_cron}", prompt: "/atelier:autopilot/ci-fix")`
7. `CronCreate(cron: "{qa_boost_cron}", prompt: "/atelier:autopilot/qa-boost")`
8. `CronCreate(cron: "{work_ledger_cron}", prompt: "/atelier:autopilot/work-ledger")` — gap-backlog / qa-backlog / ci-backlog ledger reader (Step 2.5에서 스냅샷으로 보여주는 3개 backlog의 Ready task를 claim해 issue-implementer로 디스패치)
9. `CronCreate(cron: "{release_stale_cron}", prompt: "/atelier:autopilot/stale-task-review --before {stale_wip.threshold}")` — stale Wip 관찰 + 에이전트 리뷰 (worker crash / ctrl-C / worktree 파괴 복구). CLI (`autopilot task list-stale`) 는 cutoff 보다 오래된 Wip 후보를 read-only 로 반환하고, `stale-task-reviewer` 에이전트가 task 별로 release / fail / escalate / leave alone 을 결정합니다. exit 0 (idempotent — 후보가 0건이어도 정상). 결정은 컨텍스트 의존이므로 CLI 가 아닌 에이전트가 수행합니다 (CLAUDE.md "책임 경계").

test_watch 배열이 비어있지 않으면 각 스위트별 루프 추가:
```
CronCreate(cron: "{suite_interval_cron}", prompt: "/atelier:autopilot/test-watch {suite.name}")
```
...
|------|---------|----------|------|
| Build Issues | /atelier:autopilot/build-issues | {interval} | {cron} |
| Monitor Health | health check | 30m | */30 * * * * |
| Test: {name} | /atelier:autopilot/test-watch {name} | {interval} | {cron} |
...
| Loop | Command | Interval | Cron |
|------|---------|----------|------|
| Gap Watch | /atelier:autopilot/gap-watch | 30m | */30 * * * * |
| Analyze Issue | /atelier:autopilot/analyze-issue | 20m | */20 * * * * |
| Build Issues | /atelier:autopilot/build-issues | 15m | */15 * * * * |
| Merge PRs | /atelier:autopilot/merge-prs | 10m | */10 * * * * |
| CI Watch | /atelier:autopilot/ci-watch | 20m | */20 * * * * |
| CI Fix | /atelier:autopilot/ci-fix | 15m | */15 * * * * |
| QA Boost | /atelier:autopilot/qa-boost | 1h | 7 * * * * |
| Work Ledger | /atelier:autopilot/work-ledger | 10m | */10 * * * * |
| Stale Task Review | /atelier:autopilot/stale-task-review --before 1h | 30m | */30 * * * * |
| Test: e2e | /atelier:autopilot/test-watch e2e | 2h | 7 */2 * * * |
