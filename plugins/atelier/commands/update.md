...
- exit code: `0` 드리프트 없음 / `1` 드리프트 발견 / `2` 스크립트 오류. **exit 1 은 실패가 아니라 Step 3 의 입력입니다**

### Step 3 — 드리프트 갱신 적용

Step 2 결과의 산출물별 STATUS 에 따라 처리합니다. 갱신의 실제 쓰기는 전부 `sync-artifact.sh` 에 위임합니다 (쓰기 전 `<file>.bak-<timestamp>` 백업, 대상 미설치·마커 손상 시 exit 2 로 거부).

**`claude-md-coding-style-block=DRIFTED`** → 질문 없이 바로 갱신합니다. 마커 구간은 `DO NOT REMOVE` 로 플러그인 소유가 선언된 영역이고, 마커 밖 사용자 내용은 보존됩니다:

```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/sync-artifact.sh" --target claude-md
```
...
- **덮어쓰기** 선택 시:
  ```bash
  bash "${CLAUDE_PLUGIN_ROOT}/scripts/sync-artifact.sh" --target rules --project-dir "${CLAUDE_PROJECT_DIR:-.}"
  ```
...
- `OK` → 최신 상태임을 알립니다
- 갱신됨 → sync-artifact.sh 출력의 백업 경로를 함께 알립니다
...
- **sync-artifact.sh exit 2** (`ERROR: ...`) → 대상 미설치 또는 마커 손상입니다. 직접 파일을 고치려 들지 말고 stderr 원문과 함께 `/atelier:setup` 재실행을 안내합니다.

## Output Examples

**드리프트 없음 (check-drift exit 0 — Step 3 없음):**
