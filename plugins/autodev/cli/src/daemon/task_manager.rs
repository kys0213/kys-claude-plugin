...
//!
//! Collector에서 Task를 수집하고, Daemon에게 실행 가능한 Task를 제공한다.
...
/// Task 수집 및 분배 관리자.
///
/// Daemon과 Collector 사이의 중재자 역할:
/// - `tick()`: 주기적으로 Collector를 poll하여 Task 수집
...
pub trait TaskManager: Send {
    /// 주기적 하우스키핑.
    /// 모든 Collector를 poll하여 새 Task를 수집한다.
    ...
    /// 완료된 Task 결과를 모든 Collector에 반영한다.
