...
도구가 정교해야 소비자(에이전트, 사람, 스크립트)가 단순해진다.

- 소비자는 **"무엇을"** 표현하고, **"어떻게"**는 도구가 책임진다
- 내부 구현(경로, 필드 구조, 동기화)을 외부에 노출하지 않는다
- 반복되는 인프라 관심사(lock, 환경변수, 로깅)는 도구에 내장한다

### 큐 상태 머신

daemon, Claw, Executor의 역할이 명확히 분리된 3단계 흐름:

```
1. Collector (daemon):  외부 소스 감지 → 큐에 Pending으로 저장
2. Claw (headless/interactive): 큐 평가 → advance/skip/hitl 판단
3. Executor (daemon):   Ready 작업 꺼내서 실행 → 결과 저장

          Collector          Claw             Executor
            │                 │                 │
  소스 감지 ─→ Pending ──────→ Ready ──────────→ Running
            (daemon)      (advance)          (daemon)
                            │                   │
                          Skip               Done/Failed
                        (판단)              (결과 저장)
```

- **Daemon은 판단하지 않는다**: Pending에 넣기만 하고, Ready를 실행하기만 함
- **Claw만 advance/skip 판단**: headless(cron) 또는 interactive(autodev agent)로
- **Queue phase**: `Pending → Ready → Running → Done/Failed`

### Collector의 소스별 상태 매핑

각 Collector는 외부 소스의 상태 표현을 **통합 큐 phase로 변환**한다:

```
GitHub:   라벨 (autodev:analyze, autodev:wip, ...)  → Pending
Slack:    이모지 (👀, ✅, ❌, ...)                   → Pending
Jira:     상태 필드 (To Do, In Progress, ...)       → Pending
                    ↓
            Collector가 변환
                    ↓
Queue:    Pending → Ready → Running → Done/Failed (통합 phase)
```

상태 매핑은 Collector 구현체의 책임 (OCP).
...
사용자가 `autodev agent`를 안 띄워도 cron이 headless로 Claw를 호출하여 자율 루프가 동작한다.

### Headless Claw의 다중 레포 판단

```bash
# claw-evaluate.sh 내부 흐름:
# 1. 전체 레포 큐 조회
autodev queue list --json
# → [{"repo":"org/repo-a","items":[...]}, {"repo":"org/repo-b","items":[...]}]

# 2. Claw가 전체를 보고 레포별로 판단
#    "repo-a의 #42는 advance, repo-b의 #13은 skip"

# 3. 레포별 advance/skip 실행
autodev queue advance issue:org/repo-a:42
autodev queue skip issue:org/repo-b:13
```
...
# .autodev.yaml
claw:
  schedule_interval_secs: 60    # → claw-evaluate cron의 기본 주기
  gap_detection_interval_secs: 3600  # → gap-detection cron의 기본 주기
```

`claw.schedule_interval_secs`는 `claw-evaluate` cron job의 **기본 interval 설정값**이다.
`autodev cron update claw-evaluate --interval 120`으로 개별 조정 가능.
...
Heartbeat (tick_interval_secs, 기본 10초):
  → 가볍고 빠른 반복
  1. Collector.collect() → Pending으로 큐에 저장 (수집만)
  2. Queue에서 "Ready" 상태 작업 꺼내서 실행 (실행만)
  3. 실행 결과 → DB에 저장 (저장)

Cron (등록된 스크립트 주기 실행):
  → claw-evaluate, gap-detection 등
  → 환경변수 주입 + 스크립트 실행
```
...
#### Global (레포 무관, 결정적 로직 — LLM 불필요)

Global job은 초기 설정 시 1회 등록되며, guard + CLI 명령으로만 처리한다.
`autodev agent` (= `claude -p`) 호출 없이 결정적 로직만 수행한다.

| Job | 기본 주기 | Guard | 동작 |
|-----|----------|-------|------|
| **hitl-timeout** | 5분 | 미응답 HITL 있을 때 | `autodev hitl timeout` — 타임아웃 초과 HITL 만료 처리 |
| **daily-report** | 매일 06시 | daemon 로그에 활동 있을 때 | daemon 내장 `DailyReporter` (로그 파싱 → 통계 → GitHub 이슈) |
| **log-cleanup** | 매일 00시 | 보관 기간 초과 로그 있을 때 | `find -mtime +N -delete` |

#### Per-repo (레포 컨텍스트 필요, LLM 판단 필요)

per-repo job은 `autodev agent --repo <name>`으로 실행되므로 레포 정보가 필수.
...
│   ├── claw-evaluate.sh        ← guard + autodev agent (LLM)
│   ├── gap-detection.sh        ← guard + autodev agent (LLM)
│   ├── knowledge-extract.sh    ← guard + autodev agent (LLM)
│   ├── hitl-timeout.sh         ← guard + autodev CLI (결정적)
│   ├── daily-report.sh         ← guard + daemon 내장 (결정적)
│   └── log-cleanup.sh          ← guard + autodev CLI (결정적)
│
...
daemon은 **환경변수 주입 + 스크립트 실행기 + 중복 실행 방지**. guard도 명령도 전부 스크립트 안에 있다.
이전 실행이 완료되지 않은 job은 다음 틱에서 자동 skip된다 (in-memory 상태 관리).
...
```
claw-evaluate 실행 시:
  autodev agent -p "큐를 평가하고 다음 작업을 결정해줘"
