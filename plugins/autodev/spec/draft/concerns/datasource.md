...
- `get_context()`: 해당 아이템의 외부 시스템 컨텍스트를 조회한다 (리뷰 코멘트, 피드백, PR status 등). 코어 evaluate가 분류 판단 시 호출.
...
on_done은 두 가지 타입을 지원한다:

```yaml
states:
  analyze:
    on_done:
      - label: "autodev:implement"              # 라벨 부착 (선언적)
      - script: "scripts/post-analyze.sh"       # 스크립트 실행 (autodev CLI 호출 포함)

  implement:
    on_done:
      - label: "autodev:review"
      - script: "scripts/post-implement.sh"     # PR 생성, 브랜치 관리 등
```

script 내부에서 `autodev` CLI를 호출하여 복합 액션을 수행할 수 있다. 라벨만으로 표현할 수 없는 부수효과(PR 생성, 코멘트 작성, 브랜치 머지 등)는 script에서 처리.
