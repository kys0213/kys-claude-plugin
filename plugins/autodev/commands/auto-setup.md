...
| 선택 | `develop-workflow` | `kys-claude-plugin` |
...
**Case 1: 리뷰 플러그인 감지됨 (`develop-workflow` 또는 `review`)**

AskUserQuestion으로 질문:
- Issue 분석 워크플로우:
  - `builtin` — autodev 내장 에이전트 (기본, 외부 의존성 없음) (권장)
  - 감지된 커스텀 커맨드 (예: `/develop-workflow:develop-auto`)
- PR 리뷰 워크플로우:
  - `builtin` — autodev 내장 에이전트 (기본, 외부 의존성 없음) (권장)
  - 감지된 커스텀 커맨드 (예: `/develop-workflow:multi-review`)
...
> 커스텀 워크플로우를 사용하려면 `develop-workflow` 등 리뷰 플러그인을 설치 후 재설정하세요.
