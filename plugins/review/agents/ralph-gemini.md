---
name: ralph-gemini
description: RALPH 루프용 Google Gemini 코드 리뷰 에이전트 - 테스트 전 코드 품질 검토
whenToUse: |
  다음 상황에서 이 에이전트를 사용하세요:
  - /ralph-review 실행 시 (Gemini용)
...
# RALPH Gemini 리뷰 에이전트

RALPH 피드백 루프에서 Google Gemini를 사용하여 코드를 검토합니다.
...
```bash
Read .review-output/gemini-TIMESTAMP.txt
```
...
```
Error: Google Gemini 스크립트 실행 실패
