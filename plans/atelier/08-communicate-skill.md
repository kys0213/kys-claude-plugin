...
`spec-write`·`git` 을 동시 등록하고 `claude -p` 를 140회 돌려 어느 스킬이 발동하는지 셌다.

| 관측 | 결과 |
|---|---|
| `orchestrator` 가 가져간 횟수 | 140회 중 **1회** |
| `git` 이 "PR 설명 써줘" 를 가져간 횟수 | 40회 중 **28회** |
| 발동하면 안 되는 요청(버그 수정·테스트 추가)에서의 오발동 | 0/20 |

가설과 실제가 어긋났다. 경합은 `orchestrator` 가 아니라 **`git` 과의 사이에서** 일어나고,
PR 본문 요청에서는 `git` 이 대부분 이긴다. 3.3 의 로딩 계층 덕분에 기준선은 CLAUDE.md 로
항상 실려 최소 품질이 지켜지지만, 작문 기준 자체는 로드되지 않는다.

description 압축(#850, 909자 → 623자)은 발동률에 측정 가능한 차이를 만들지 않았다 — 단독
27/30 vs 26/30, 경쟁 26/30 vs 22/30, 표적 n=15 에서 11/30 vs 13/30 으로 방향이 일정하지 않다.
n=5 에서 보이던 쿼리별 차이는 n=15 에서 모두 사라졌다.
